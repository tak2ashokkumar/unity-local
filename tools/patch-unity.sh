...
const ssPath = path.join(rootDir, 'uldb/ngx-unity/static-server.js');
...
const proxyRules = proxyConfPath
    ...
    // 2. Inject proxy handler before the /static/ route
    const proxyHandler = `
    /**
...
const pkgPath = path.join(rootDir, 'uldb/ngx-unity/package.json');
