...
import { AIMLSuppressionRule, AIMLSuppressionRuleCondition } from 'src/app/shared/SharedEntityTypes/aiml-rules.type';
import { NoWhitespaceValidator } from 'src/app/shared/app-utility/app-utility.service';

@Injectable()
export class MtpAimlSuppressionRulesCrudService {
  ...
  createRuleForm(ruleId?: any, readOnly?: boolean): Observable<FormGroup> {
    if (ruleId) {
      return this.http.get<AIMLSuppressionRule>(`/customer/mtp/srules/${ruleId}/`).pipe(
        map(rule => {
  ...
  createRule(rule: AIMLSuppressionRule) {
    return this.http.post(`/customer/mtp/srules/`, this.getCrudType(rule));
  }

  editRule(rule: AIMLSuppressionRule) {
    return this.http.put(`/customer/mtp/srules/${rule.uuid}/`, this.getCrudType(rule));
  }
}
