...
@Injectable()
export class MtpAimlSummaryService {
  ...
  convertToSummaryViewdata(summary: MTPAIMLSummary): AIMLSummaryViewData {
  ...
  getEventsCount(tenants: string[], formData: any) {
    let obj = { count_by: formData.target_type.key, tenants: tenants };
    return this.http.post<any[]>(`customer/mtp/events/count/`, obj);
  }
  ...
  convertToEventsCountChartData(eventData: any[]) {
    ...
    view.options = this.chartConfig.getDefaultPieChartOptions();
    view.options.legend = {
      position: 'right',
      labels: { padding: 2, boxWidth: 25 },
  ...
  convertToAlertsCountViewdata(alertData: MTPAlertCountByDeviceType[]): AIMLSummaryAlertsCountViewData {
  ...
  getNoisyEvents(tenants: string[]) {
    return this.http.post<any[]>(`customer/mtp/events/noisy/`, { tenants: tenants });
  }

  convertToNoisyEventsViewdata(events: any[]): AIMLSummaryNoisyEventsViewData[] {
    ...
    events.map(al => {
      ...
      a.source = al.source;
      a.lastReported = al.last_reported ? datePipe.transform(al.last_reported.replace(/\s/g, "T"), environment.unityDateFormat) : 'N/A';
  ...
  getNoisyHosts(tenants: string[]) {
    return this.http.post<any[]>(`customer/mtp/events/noisy_host/`, { tenants: tenants });
  }

  convertToNoisyHostsViewData(hostData: any[]): AIMLSummaryNoisyHostsViewData[] {
    let maxValue = 0;
    hostData.map(hd => {
