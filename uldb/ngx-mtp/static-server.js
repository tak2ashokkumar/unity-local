...
const server = http.createServer(function (req, res) {
    ...
    const isGzipSupported = acceptEncoding && acceptEncoding.includes('gzip');

    if (isGzipSupported && req.url.endsWith('css') || req.url.endsWith('js')) {
        ...
        fs.stat(gzFilePath, function (err, stat) {
            if (err || !stat.isFile()) {
                ...
                log('gzip not found serving uncompressed');
        ...
        log('gzip not supported serving uncompressed');
        res.setHeader('Cache-Control', 'no-cache');
        res.setHeader('Expires', 0);
        serve(req, res, done);
    }
