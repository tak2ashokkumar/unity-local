...
/**
 * Data service for the Bank of Anthos Application Dashboard.
 *
 ...
 */
@Injectable()
export class BankOfAnthosApplicationDashboardService {
  ...
  getSessionToOrderFunnel(appId: number, selectedDateRangeFormData: DurationDropdownType) {
    ...
    return this.http.get<{ [key: string]: number }>(`/apm/app_list/${appId}/boa_funnels/`, { params });
  }
  ...
  getReturningCustomerCategory(appId: number, selectedDateRangeFormData: DurationDropdownType) {
    const params = this.converteDropdownsDataToApiParamsData(selectedDateRangeFormData);
    return this.http.get<{ [key: string]: number }>(`/apm/app_list/${appId}/boa_error_analysis/`, { params });
  }
  ...
  getNewCustomers(appId: number, selectedDateRangeFormData: DurationDropdownType) {
    const params = this.converteDropdownsDataToApiParamsData(selectedDateRangeFormData);
    return this.http.get<{ [key: string]: any }>(`/apm/app_list/${appId}/boa_daily_active_users/`, { params });
  }
   ...
   *  - Carts     1520, interpolated via `log10(carts/orders)` so the gap between
  ...
  convertToNewCustomersChartData(graphData: { [key: string]: any }): UnityChartDetails {
    const activeUsers = graphData?.active_users || graphData?.daily_active_users || graphData?.new_customers || [];
    ...
    activeUsers.forEach(d => xxisData.push(d.range || d.period || d.date));
    activeUsers.forEach(d => yxisData.push(d.total ?? d.sum ?? d.count ?? d.value));
    view.options.title = {
      text: 'Active Users', left: 'center', top: '0%',
      textStyle: { fontFamily: UNITY_FONT_FAMILY(), fontSize: 13, fontWeight: 500, color: UNITY_TEXT_DEFAULT_COLOR() }
    };
  ...
  getCheckoutAbondanRate(appId: number, selectedDateRangeFormData: DurationDropdownType) {
    const params = this.converteDropdownsDataToApiParamsData(selectedDateRangeFormData);
    return this.http.get<{ [key: string]: string }>(`/apm/app_list/${appId}/boa_success_rates/`, { params });
  }
  ...
  getConversionRate(appId: number, selectedDateRangeFormData: DurationDropdownType) {
    const params = this.converteDropdownsDataToApiParamsData(selectedDateRangeFormData);
    return this.http.get<any>(`/apm/app_list/${appId}/boa_signup_failure_rate/`, { params });
  }
  ...
  convertToConversionRateChartData(graphData: { [key: string]: string }): UnityChartDetails {
    ...
    view.options.title = {
      text: 'Signup failure rate', left: 'center', top: '0%',
      textStyle: { fontFamily: UNITY_FONT_FAMILY(), fontSize: 13, fontWeight: 500, color: UNITY_TEXT_DEFAULT_COLOR() }
    };
  ...
  convertApplicationResponseTimeChartData(data: any[]): UnityChartDetails {
    ...
    view.options = this.getCommonLineChartOptions(view.options, labels, values, 'Application resp time (ms)', 'Response Time (ms)', 'Response Time', '#5B8FF9');
    return view;
  }
  ...
  convertErrorRateChartData(data: any[]): UnityChartDetails {
    ...
    const labels = data.map(d => d.range);
    const values = data.map(d => d.sum);
    view.options = this.getCommonLineChartOptions(view.options, labels, values, 'Services error rate (% of Failed Interactions)', 'Error Rate (%)', 'Services error rate', '#FF6B6B');
    return view;
  }
  ...
  convertPyamentFailureChartData(data: any[]): UnityChartDetails {
    ...
    const labels = data.map(d => d.range);
    const values = data.map(d => d.sum);
    view.options = this.getCommonLineChartOptions(view.options, labels, values, 'Application error rate (%)', 'Error Rate (%)', 'Application error rate', '#FFA940');
    return view;
  }
