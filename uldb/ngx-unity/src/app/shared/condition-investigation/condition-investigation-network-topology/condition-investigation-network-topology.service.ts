...
@Injectable()
export class ConditionInvestigationNetworkTopologyService {
  ...
  getNetworkTopologyData(device: any) {
    return this.http.get(`/internal/mcp/topology/?view=pc_view&customer_id=${device?.customer_id}&user_id=${this.userInfoService.userDetails.id}&device_uuid=${device?.device_uuid}`);
  }
