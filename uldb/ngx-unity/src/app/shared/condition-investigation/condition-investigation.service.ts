...
import { ConditionData, PromptResult } from './condition-investigation.type';
...
export class ConditionInvestigationService {
  ...
  onConditionAcknowledge(conditionUuid: string, formData: any) {
    return this.http.post<ConditionDetailsType>(`/customer/aiops/conditions/${conditionUuid}/acknowledge/`, formData)
      .pipe(map(res => {
...
export class ConditionDetailsViewData {
  constructor() { }
  id: number;
  uuid: string;
  ruleName: string;
...
export interface ConditionDetailsType {
  id: number;
  uuid: string;
  rule_name: string;
  description: string;
