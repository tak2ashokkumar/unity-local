...
import { KubernetesEventsService, KubernetesEventsViewdata } from './kubernetes-events.service';

@Component({
  selector: 'kubernetes-events',
  templateUrl: './kubernetes-events.component.html',
  styleUrls: ['./kubernetes-events.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [KubernetesEventsService]
})
export class KubernetesEventsComponent implements OnInit, OnDestroy {
  private readonly ngUnsubscribe = new Subject<void>();

  count: number = 0;
  viewData: KubernetesEventsViewdata[] = [];
  controllerId: string;
  currentCriteria: SearchCriteria;
  showMonitoring: boolean;

  constructor(private route: ActivatedRoute,
    private svc: KubernetesEventsService,
  ...
  trackByUuid(index: number, item: KubernetesEventsViewdata): string {
    return item.uuid;
  }

  goToStats(view: KubernetesEventsViewdata) {
    this.k8sMon.goToStats(this.router, this.route, DeviceMapping.KUBERNETES_EVENT, view.uuid, view.name, view.monitoring);
  }

  private getEvents() {
    this.spinnerSvc.start('main');
    this.svc.getEvents(this.controllerId, this.currentCriteria).pipe(
      ...
      .subscribe(data => {
        ...
        this.notificationSvc.error(new Notification('Error while fetching Events.'));
      });
  }
  ...
  private syncEvents() {
    ...
    this.spinnerSvc.start('main');
    this.svc.syncEvents(this.controllerId).pipe(
      takeUntil(this.ngUnsubscribe),
      switchMap(() => this.svc.getEvents(this.controllerId, this.currentCriteria)),
      finalize(() => { this.refreshSvc.stop(); this.stopSpinnerAndMarkForCheck(); }))
      .subscribe(data => {
        this.count = data.count;
        this.viewData = this.svc.convertToViewdata(data.results);
      }, err => {
        this.notificationSvc.error(new Notification('Error while syncing Events.'));
      });
  }
