...
import { ChartConfigService, UnityChartData } from '../chart-config.service';
import { Observable } from 'rxjs';
import moment from 'moment';
import { GET_INTERFACE_DETAILS_BY_DEVICE_TYPE, ZABBIX_DEVICE_GRAPH_IMAGE } from '../api-endpoint.const';
import { interfaceAlertDetailsType, interfaceDetailsType, interfaceRecentAlertsType, interfaceSummaryType } from '../SharedEntityTypes/interface-details.type';

@Injectable({
  providedIn: 'root'
})
export class SharedInterfaceDetailsService {
  ...
  getInterfaceDetails(deviceType: DeviceMapping, deviceId: string, interfaceId: string): Observable<interfaceDetailsType> {
    return this.http.get<interfaceDetailsType>(GET_INTERFACE_DETAILS_BY_DEVICE_TYPE(deviceType, deviceId, interfaceId));
  }

  convertToInterfaceSummaryData(data: interfaceSummaryType): DeviceInterfaceDetailsViewData {
  ...
  convertToInterfaceAlertDetailsChartData(data: interfaceAlertDetailsType): UnityChartData {
  ...
  convertToInterfaceRecentAlertsViewData(alerts: interfaceRecentAlertsType[]): DeviceInterfaceRecentAlertsViewData[] {
...
export class DeviceInterfaceDetailsGraphViewdata {
  constructor() { }
  graphId: string;
  name: string;
  image: string;
