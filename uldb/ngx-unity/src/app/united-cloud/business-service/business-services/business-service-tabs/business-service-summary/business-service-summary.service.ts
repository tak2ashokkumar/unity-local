...
export class BusinessServiceSummaryService {
  ...
  buildForm(buId: string): FormGroup {
  ...
  getSummary(selectedApps: number[]): Observable<BUCostCenterApplicationSummary> {
    let params: HttpParams = new HttpParams();
    if (selectedApps && selectedApps.length) {
      selectedApps.forEach(appId => {
        params = params.append('app_id', appId.toString());
      });
    }
    return this.http.get<BUCostCenterApplicationSummary>(`apm/business_summary/app_info/`, { params: params });
  }

  getThroughputData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string, type: string): Observable<any> {
  ...
  getLatencyData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string, type: string): Observable<any> {
    const params = this.converteDropdownsDataToApiParamsData(dropdownsViewData, selectedApps, key, type);
    return this.http.get<any>(`apm/business_summary/data/`, { params: params });
  }

  getAvailabilityData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string, type: string): Observable<any> {
    const params = this.converteDropdownsDataToApiParamsData(dropdownsViewData, selectedApps, key, type);
    return this.http.get<any>(`apm/business_summary/data/`, { params: params });
  }

  getResponseTimeData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string, type: string): Observable<any> {
    const params = this.converteDropdownsDataToApiParamsData(dropdownsViewData, selectedApps, key, type);
    return this.http.get<any>(`apm/business_summary/data/`, { params: params });
  }

  getHostOverview(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string): Observable<any> {
  ...
  convertToApplicationChartViewData(apps: any, dropdownsViewData: DropDownsViewData, yAxisLabelName: string, xAxisLabelName: string) {
    if (!apps) {
      return null;
    }
  ...
  convertToServiceChartViewData(apps: any, dropdownsViewData: DropDownsViewData, yAxisLabelName: string, xAxisLabelName: string, type?: string) {
    if (!apps) {
      return null;
    }
  ...
  convertToComponentChartViewData(apps: any, dropdownsViewData: DropDownsViewData, yAxisLabelName: string, xAxisLabelName: string, type?: string) {
    if (!apps) {
      return null;
    }
  ...
  convertToCpuUtilizationChartViewData(
    ...
    return this.convertToAreaChartViewData(
      ...
      names,
      colors,
      true  // smooth lines for area chart
    );
  }

  convertToAreaChartViewData(xAxisData: string[], yAxisLabelName: string, values: number[][], names: string[], colors: string[], smoothLines: boolean) {
    let view: UnityChartDetails = new UnityChartDetails();
    ...
    if (names?.length && values?.length) {
      ...
      view.options.legend = { 
        data: names, 
        bottom: 0, 
        type: names.length > 12 ? "scroll" : "plain" 
      };
    }
    ...
    view.options.title = {
      text: yAxisLabelName ? `${yAxisLabelName} vs Time` : '',
      left: 'center',
      top: 10,
      textStyle: {
        fontSize: 16,
  ...
  convertToComponentDoughnutChartViewData(apps: any, dropdownsViewData: DropDownsViewData, yAxisLabelName: string, xAxisLabelName: string, type?: string) {
    if (!apps) {
      return null;
    }
  ...
  convertToLineChartViewData(xAxisData: string[], yAxisLabelName: string, xAxisLabelName: string, values?: number[][], names?: string[], colors?: string[], smooth?: string, enableGlow?: boolean) {
    let view: UnityChartDetails = new UnityChartDetails();
  ...
  converteDropdownsDataToApiParamsData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string, type: string): HttpParams {
    let params: HttpParams = new HttpParams();
    // append app_ids
    if (selectedApps && selectedApps.length) {
      selectedApps.forEach(appId => {
  ...
  converteDropdownsDataToApiParamsForHostData(dropdownsViewData: DropDownsViewData, selectedApps: number[], key: string): HttpParams {
    let params: HttpParams = new HttpParams();
    // append app_ids
    if (selectedApps && selectedApps.length) {
      selectedApps.forEach(appId => {
  ...
  getCloudList(appIdList: any[]): Observable<any> {
    let params = new HttpParams();

    if (appIdList && appIdList.length) {
      appIdList.forEach(id => {
        params = params.append('app_id', id);
      });
    }
...
export class ApplicationOverviewViewData {
  constructor() { }
  throughputLoader: string = 'applicationOverviewThroughputLoader';
  availabilityLoader: string = 'applicationOverviewAvailabilityLoader';
  responseTimeLoader: string = 'applicationOverviewResponseTimeLoader';
...
export class ServicesOverviewViewData {
  constructor() { }
  throughputLoader: string = 'servicesOverviewThroughputLoader';
  availabilityLoader: string = 'servicesOverviewAvailabilityLoader';
  responseTimeLoader: string = 'servicesOverviewResponseTimeLoader';
  throughputAvg: string;
  availabilityAvg: string;
  responseTimeAvg: string;
  throughputChartData: UnityChartDetails;
...
export class MetricsViewResponse {
  constructor() { };
  data: MetricsData;
}
