...
@Component({
  selector: 'zabbix-dc-pdu-graph-crud',
  templateUrl: './zabbix-dc-pdu-graph-crud.component.html',
  styleUrls: ['./zabbix-dc-pdu-graph-crud.component.scss'],
  providers: [ZabbixDcPduGraphCrudService]
})
export class ZabbixDcPduGraphCrudComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();

  deviceId: string;
  viewData: DcPDUMonitoringGraphsViewdata[] = [];
  filteredViewData: DcPDUMonitoringGraphsViewdata[] = [];
  fieldsToFilterOn: string[] = ['name', 'graphType'];
  currentCriteria: SearchCriteria;

  selectedGraph: DcPDUMonitoringGraphsViewdata;
  graphItems: DcPDUZabbixMonitoringGraphItems[] = [];
  nonFieldErr: string = '';
  action: 'Add' | 'Edit';
  ...
  constructor(private graphService: ZabbixDcPduGraphCrudService,
  ...
  createTicket(graph: DcPDUMonitoringGraphsViewdata) {
    this.ticketService.createTicket({
      subject: DATABASE_GRAPH_TICKET_SUBJECT(graph.name),
      metadata: DATABASE_GRAPH_TICKET_METADATA(DeviceMapping.PDU, graph.name)
    }, DeviceMapping.PDU);
  }
  ...
  buildAddEditForm(graph?: DcPDUMonitoringGraphsViewdata) {
  ...
  editGraph(view: DcPDUMonitoringGraphsViewdata) {
    if (!view.canEdit) {
      return;
    }
    this.action = 'Edit';
  ...
  deleteGraph(view: DcPDUMonitoringGraphsViewdata) {
    if (!view.canDelete) {
      return;
    }
    this.selectedGraph = view;
