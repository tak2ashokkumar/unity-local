...
import { AzureZabbixMonitoringGraphItems } from '../azure-zabbix-monitoring.type';
import { AzureMonitoringGraphsViewdata, ZabbixAzureGraphsCrudService } from './zabbix-azure-graphs-crud.service';

@Component({
  selector: 'zabbix-azure-graphs-crud',
  templateUrl: './zabbix-azure-graphs-crud.component.html',
  styleUrls: ['./zabbix-azure-graphs-crud.component.scss'],
  providers: [ZabbixAzureGraphsCrudService]
})
export class ZabbixAzureGraphsCrudComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();

  deviceId: string;
  viewData: AzureMonitoringGraphsViewdata[] = [];
  filteredViewData: AzureMonitoringGraphsViewdata[] = [];
  fieldsToFilterOn: string[] = ['name', 'graphType'];
  currentCriteria: SearchCriteria;
  device: DeviceTabData;

  selectedGraph: AzureMonitoringGraphsViewdata;
  graphItems: AzureZabbixMonitoringGraphItems[] = [];
  nonFieldErr: string = '';
  action: 'Add' | 'Edit';
  ...
  constructor(private graphService: ZabbixAzureGraphsCrudService,
  ...
  createTicket(graph: AzureMonitoringGraphsViewdata) {
    this.ticketService.createTicket({
      subject: DATABASE_GRAPH_TICKET_SUBJECT(graph.name),
      metadata: DATABASE_GRAPH_TICKET_METADATA(DeviceMapping.AZURE_ACCOUNTS, graph.name)
    }, DeviceMapping.AZURE_ACCOUNTS);
  }

  getGraphItems() {
    this.graphService.getGraphItems(this.device.deviceType, this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.graphItems = res;
    }, (err: HttpErrorResponse) => {
    });
  }

  buildAddEditForm(graph?: AzureMonitoringGraphsViewdata) {
  ...
  confirmCreate() {
    if (this.graphForm.invalid) {
      ...
      if (this.selectedGraph) {
        this.graphService.updateGraph(this.device.deviceType, this.deviceId, this.selectedGraph.graphid, this.graphForm.getRawValue()).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
          this.graphModelRef.hide();
          this.spinner.stop('main');
        ...
        this.graphService.createGraph(this.device.deviceType, this.deviceId, this.graphForm.getRawValue()).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
          this.graphModelRef.hide();
          this.spinner.stop('main');
  ...
  editGraph(view: AzureMonitoringGraphsViewdata) {
    if (!view.canEdit) {
      return;
    }
    this.action = 'Edit';
  ...
  deleteGraph(view: AzureMonitoringGraphsViewdata) {
    if (!view.canDelete) {
      return;
    }
    this.selectedGraph = view;
  ...
  confirmDelete() {
    ...
    this.spinner.start('main');
    this.graphService.deleteGraph(this.device.deviceType, this.deviceId, this.selectedGraph.graphid).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.spinner.stop('main');
