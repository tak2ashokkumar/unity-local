...
import { DEVICE_DATA_BY_DEVICE_TYPE, GET_DEVICE_GRAPH_BY_TYPE, GET_DEVICE_SENSOR_BY_DEVICETYPE, GET_DEVICE_STATUS_BY_DEVICETYPE } from 'src/app/shared/api-endpoint.const';
import { DeviceGraphTypeMapping, DeviceMapping } from 'src/app/shared/app-utility/app-utility.service';
...
@Injectable()
export class BmServersOverviewService {

  constructor(private http: HttpClient) { }

  getDeviceData(deviceId: string): Observable<DeviceData> {
    return this.http.get<DeviceData>(DEVICE_DATA_BY_DEVICE_TYPE(DeviceMapping.BARE_METAL_SERVER, deviceId), { headers: Handle404Header });
  }
  ...
  getDeviceStatus(deviceId: string): Observable<DeviceStatus> {
    ...
    return this.http.get<DeviceStatus>(GET_DEVICE_STATUS_BY_DEVICETYPE(DeviceMapping.BARE_METAL_SERVER, deviceId));
  }
  ...
  getDeviceSensors(deviceId: string): Observable<APIDeviceSensor> {
    ...
    //   ]
    // })
    return this.http.get<APIDeviceSensor>(GET_DEVICE_SENSOR_BY_DEVICETYPE(DeviceMapping.BARE_METAL_SERVER, deviceId));
  }

  getProcessorGraph(deviceId: string): Observable<DeviceGraphData> {
    return this.http.get<DeviceGraphData>(GET_DEVICE_GRAPH_BY_TYPE(deviceId, DeviceGraphTypeMapping.PROCESSOR, DeviceMapping.BARE_METAL_SERVER));
  }

  getPortsGraph(deviceId: string): Observable<DeviceGraphData> {
    return this.http.get<DeviceGraphData>(GET_DEVICE_GRAPH_BY_TYPE(deviceId, DeviceGraphTypeMapping.PORTS, DeviceMapping.BARE_METAL_SERVER));
  }

  getMemoryGraph(deviceId: string): Observable<DeviceGraphData> {
    return this.http.get<DeviceGraphData>(GET_DEVICE_GRAPH_BY_TYPE(deviceId, DeviceGraphTypeMapping.MEMORY, DeviceMapping.BARE_METAL_SERVER));
  }
}
