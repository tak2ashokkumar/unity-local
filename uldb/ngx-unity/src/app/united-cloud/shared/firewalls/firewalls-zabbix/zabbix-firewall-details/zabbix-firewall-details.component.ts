...
import { ZabbixFirewallDetailsService } from './zabbix-firewall-details.service';
import { CredentialMap } from 'src/app/app-constants';

@Component({
  selector: 'zabbix-firewall-details',
  templateUrl: './zabbix-firewall-details.component.html',
  styleUrls: ['./zabbix-firewall-details.component.scss'],
  providers: [ZabbixFirewallDetailsService]
})
export class ZabbixFirewallDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private detailService: ZabbixFirewallDetailsService,
  ...
  async getDeviceDetails() {
    ...
    this.detailService.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.getCredentials();
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.FIREWALL, configured: res.monitoring.configured }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
