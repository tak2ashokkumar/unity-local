...
@Injectable()
export class ZabbixFirewallDetailsService {
  constructor(private http: HttpClient,
    private builder: FormBuilder,) { }

  getDropdownData() {
    const manufacturers = this.http.get<FirewallCRUDManufacturer[]>(DEVICE_MANUFACTURERS(DeviceMapping.FIREWALL));
  ...
  getDeviceDetails(deviceId: string): Observable<Firewall> {
    return this.http.get<Firewall>(DEVICE_BY_ID(DeviceMapping.FIREWALL, deviceId));
  }
  ...
  getModels(manufacturer: string): Observable<Array<FirewallCRUDModel>> {
    return this.http.get<Array<FirewallCRUDModel>>(DEVICE_MODELS(DeviceMapping.FIREWALL, manufacturer));
  }
  ...
  buildDetailForm(d: Firewall): FormGroup {
  ...
  buildMetaDataForm(d: Firewall): FormGroup {
    return this.builder.group({
  ...
  buildLocationForm(d: Firewall): FormGroup {
    return this.builder.group({
  ...
  getDeviceData(deviceId: string): Observable<DeviceData> {
    return this.http.get<DeviceData>(ZABBIX_DEVICE_DATA_BY_DEVICE_TYPE(DeviceMapping.FIREWALL, deviceId));
  }

  updateDevice(uuid: string, data: any): Observable<Firewall> {
    return this.http.put<Firewall>(DEVICE_BY_ID(DeviceMapping.FIREWALL, uuid), data);
  }

  syncUptime(uuid: string): Observable<any> {
    return this.http.get<any>(DEVICE_UPTIME_BY_DEVICE_ID(DeviceMapping.FIREWALL, uuid));
  }

  syncSerialNumber(uuid: string): Observable<string> {
    return this.http.get<string>(DEVICE_SERIAL_NUMBER_BY_DEVICE_TYPE(DeviceMapping.FIREWALL, uuid));
  }
}
