...
export class FirewallsService {
  ...
  getFirewalls(criteria: SearchCriteria): Observable<PaginatedResult<Firewall>> {
    return this.tableService.getData<PaginatedResult<Firewall>>(DEVICE_LIST_BY_DEVICE_TYPE(DeviceMapping.FIREWALL), criteria);
  }

  getAllFirewalls(criteria: SearchCriteria): Observable<Firewall[]> {
    return this.tableService.getData<Firewall[]>(DEVICE_LIST_BY_DEVICE_TYPE(DeviceMapping.FIREWALL), criteria);
  }
  ...
  getDeviceData(device: FirewallViewData) {
    ...
    const url = device.monitoring.observium ? DEVICE_DATA_BY_DEVICE_TYPE(DeviceMapping.FIREWALL, device.deviceId) : ZABBIX_DEVICE_DATA_BY_DEVICE_TYPE(DeviceMapping.FIREWALL, device.deviceId);
        ...
        map((res: DeviceData) => {
          if (res) {
            ...
            if (device.isShared) {
              ...
              device.isStatsButtonEnabled = true;
              device.statsTooltipMessage = 'Firewall Statistics';
            }
          }
          return device;
        })
  ...
  getConsoleAccessInput(device: FirewallViewData): ConsoleAccessInput {
    return {
      label: DeviceMapping.FIREWALL, deviceType: DeviceMapping.FIREWALL, deviceId: device.deviceId,
      newTab: false, deviceName: device.name, managementIp: device.managementIp
    };
  }
}

export class FirewallViewData {
  deviceId: string;
  name: string;
  deviceStatus: string;
  model: string;
