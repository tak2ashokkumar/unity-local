...
import { ZabbixHypervisorDetailsService } from './zabbix-hypervisor-details.service';
import { CredentialMap } from 'src/app/app-constants';
import { LifeCycleStageOptions, LifeCycleStageStatusOptions } from '../../hypervisors-crud/hypervisors-crud.service';

@Component({
  selector: 'zabbix-hypervisor-details',
  templateUrl: './zabbix-hypervisor-details.component.html',
  styleUrls: ['./zabbix-hypervisor-details.component.scss'],
  providers: [ZabbixHypervisorDetailsService]
})
export class ZabbixHypervisorDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private detailService: ZabbixHypervisorDetailsService,
  ...
  async getDeviceDetails() {
    ...
    this.operatingSystems = _clone(dropdownData[2]);
    this.detailService.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.getCredentials();
      let os = 'N/A';
      let platformType = 'N/A';
      if (res.instance.os) {
        os = res.instance.os.full_name;
        platformType = res.instance.os.platform_type ? res.instance.os.platform_type : 'N/A';
      }
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.HYPERVISOR, configured: res.monitoring.configured, os: os, ssr_os: platformType }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
  ...
  submitDetailForm() {
    if (this.detailForm.invalid) {
      ...
      this.detailService.updateDevice(this.deviceId, obj).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
        this.notification.success(new Notification('Device Updated Successfully.'));
        if ((obj.credentials && ((!this.view.credentials_m2m) || this.view.credentials_m2m != obj.credentials_m2m))
          || this.view.os.id != obj.os.id || this.view.collector.uuid != obj.collector.uuid) {
