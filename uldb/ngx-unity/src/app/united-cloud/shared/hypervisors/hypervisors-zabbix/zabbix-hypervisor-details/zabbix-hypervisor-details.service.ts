...
@Injectable()
export class ZabbixHypervisorDetailsService {
  constructor(private http: HttpClient,
    private builder: FormBuilder,) { }

  getDropdownData() {
    const manufacturers = this.http.get<HypervisorCRUDManufacturer[]>(DEVICE_MANUFACTURERS(DeviceMapping.HYPERVISOR));
    const datacenters = this.http.get<DatacenterFast[]>(DEVICES_FAST_BY_DEVICE_TYPE(DeviceMapping.DC_VIZ));
    const os = this.http.get<HypervisorCRUDOperatingSystem[]>(HYPERVISOR_OS());
    return forkJoin([manufacturers, datacenters, os]);
  }

  getDeviceDetails(deviceId: string): Observable<Hypervisor> {
    return this.http.get<Hypervisor>(DEVICE_BY_ID(DeviceMapping.HYPERVISOR, deviceId));
  }
  ...
  getModels(manufacturer: string): Observable<Array<HypervisorCRUDModel>> {
    return this.http.get<Array<HypervisorCRUDModel>>(DEVICE_MODELS(DeviceMapping.HYPERVISOR, manufacturer));
  }
  ...
  buildDetailForm(d: Hypervisor): FormGroup {
    return this.builder.group({
      ...
      'out_of_band_management_ip': [d.out_of_band_management_ip, [NoWhitespaceValidator]],
      'out_of_band_management_type': [d.out_of_band_management_type, [NoWhitespaceValidator]],
  ...
  buildMetaDataForm(d: Hypervisor): FormGroup {
    return this.builder.group({
      'num_cpus': [d.num_cpus, [Validators.required, Validators.pattern("^[0-9]*$"), Validators.min(0), NoWhitespaceValidator]],
  ...
  buildLocationForm(d: Hypervisor): FormGroup {
    return this.builder.group({
  ...
  getDeviceData(deviceId: string): Observable<DeviceData> {
    return this.http.get<DeviceData>(ZABBIX_DEVICE_DATA_BY_DEVICE_TYPE(DeviceMapping.HYPERVISOR, deviceId));
  }

  updateDevice(uuid: string, data: any): Observable<Hypervisor> {
    return this.http.put<Hypervisor>(DEVICE_BY_ID(DeviceMapping.HYPERVISOR, uuid), data);
  }

  syncUptime(uuid: string): Observable<any> {
    return this.http.get<any>(DEVICE_UPTIME_BY_DEVICE_ID(DeviceMapping.HYPERVISOR, uuid));
  }

  syncSerialNumber(uuid: string): Observable<string> {
    return this.http.get<string>(DEVICE_SERIAL_NUMBER_BY_DEVICE_TYPE(DeviceMapping.HYPERVISOR, uuid));
  }
}
