...
import { SensorType, ZabbixIotDeviceSensorDetailsService } from './zabbix-iot-device-sensor-details.service';
...
@Component({
  selector: 'zabbix-iot-device-sensor-details',
  templateUrl: './zabbix-iot-device-sensor-details.component.html',
  styleUrls: ['./zabbix-iot-device-sensor-details.component.scss'],
  providers: [ZabbixIotDeviceSensorDetailsService]
})
export class ZabbixIotDeviceSensorDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private svc: ZabbixIotDeviceSensorDetailsService,
  ...
  getCabinets(dcId: string, patchValue: boolean) {
    ...
    this.svc.getCabinets(dcId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.cabinets = res;
      if (patchValue) {
        this.locationForm.patchValue({ cabinet: { id: null } });
      }
    });
  }
