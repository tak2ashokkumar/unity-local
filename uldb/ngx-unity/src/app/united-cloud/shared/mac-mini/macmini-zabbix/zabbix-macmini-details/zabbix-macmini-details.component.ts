...
import { AppUtilityService, deviceEnvironmentOptions, DeviceMapping } from 'src/app/shared/app-utility/app-utility.service';
...
import { ZabbixMacminiDetailsService } from './zabbix-macmini-details.service';
import { CredentialMap } from 'src/app/app-constants';

@Component({
  selector: 'zabbix-macmini-details',
  templateUrl: './zabbix-macmini-details.component.html',
  styleUrls: ['./zabbix-macmini-details.component.scss'],
  providers: [ZabbixMacminiDetailsService]
})
export class ZabbixMacminiDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private detailService: ZabbixMacminiDetailsService,
  ...
  async getDeviceDetails(isDeviceUpdated?: boolean) {
    if (!isDeviceUpdated) {
    ...
    this.detailService.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.getCredentials();
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.MAC_MINI, configured: res.monitoring.configured }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
