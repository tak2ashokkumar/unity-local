...
import { StorageOntapClusterDiskViewData, StorageOntapDisksService, disksColumnMapping } from './storage-ontap-disks.service';
...
@Component({
  selector: 'storage-ontap-disks',
  templateUrl: './storage-ontap-disks.component.html',
  styleUrls: ['./storage-ontap-disks.component.scss'],
  providers: [StorageOntapService, StorageOntapDisksService]
})
export class StorageOntapDisksComponent implements OnInit, OnDestroy {
  ...
  viewData: StorageOntapClusterDiskViewData[] = [];
  tableColumns: TableColumnMapping[] = disksColumnMapping;
  columnForm: FormGroup;
  columnsSelected: TableColumnMapping[] = [];
  subscr: Subscription;
  isBrokenDisk: boolean = false;
  ...
  constructor(private svc: StorageOntapDisksService,
    ...
    this.subscr = this.router.events.subscribe(event => {
      if (event instanceof NavigationEnd) {
        if (event.url.split('/').pop() == 'broken') {
          this.isBrokenDisk = true;
        } 
      }
    });
  ...
  getDisks() {
    ...
    this.svc.getDisks(this.isBrokenDisk,this.clusterId, this.currentCriteria).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  createTicket(view: StorageOntapClusterDiskViewData) {
    this.ticketService.createTicket({
      subject: TICKET_SUBJECT('Disk', view.name),
      metadata: SUMMARY_TICKET_METADATA('Disk', view.name)
    });
  }
}