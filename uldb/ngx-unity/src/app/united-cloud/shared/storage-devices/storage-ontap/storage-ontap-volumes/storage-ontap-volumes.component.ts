...
import { StorageOntapClusterVolumeViewData, StorageOntapVolumesService, volumesColumnMapping } from './storage-ontap-volumes.service';
...
@Component({
  selector: 'storage-ontap-volumes',
  templateUrl: './storage-ontap-volumes.component.html',
  styleUrls: ['./storage-ontap-volumes.component.scss'],
  providers: [StorageOntapService, StorageOntapVolumesService]
})
export class StorageOntapVolumesComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  clusterId: string;
  currentCriteria: SearchCriteria;
  count: number;
  sync: SyncResult = new SyncResult();
  tableColumns: TableColumnMapping[] = volumesColumnMapping;
  columnForm: FormGroup;
  columnsSelected: TableColumnMapping[] = [];
  viewData: StorageOntapClusterVolumeViewData[] = [];
  ...
  constructor(private svc: StorageOntapVolumesService,
  ...
  getVolumes() {
    ...
    this.svc.getVolumes(this.clusterId, this.currentCriteria).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      ...
      this.viewData = [];
      this.spinner.stop('main');
      // this.notification.error(new Notification('Failed to fetch volumes.'));
    })
  }
  ...
  loadCharts(view: StorageOntapClusterVolumeViewData) {
    this.storageService.put('ontap-entity', { name: view.name }, StorageType.SESSIONSTORAGE);
    this.router.navigate(['details'], { relativeTo: this.route });
  }

  loadDetails(view: StorageOntapClusterVolumeViewData) {
    this.storageService.put('ontap-entity', { name: view.name, type: 'volume', state: view.state}, StorageType.SESSIONSTORAGE);
    this.router.navigate([view.id, 'overview'], { relativeTo: this.route });
  }

  createTicket(view: StorageOntapClusterVolumeViewData) {
    this.ticketService.createTicket({
      subject: TICKET_SUBJECT('Volume', view.name),
      metadata: SUMMARY_TICKET_METADATA('Volume', view.name)
    });
  }
