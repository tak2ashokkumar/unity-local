...
import { AppUtilityService, CRUDActionTypes, DeviceMapping, deviceDiscoveryMethodOptions, deviceEnvironmentOptions, deviceStatusOptions } from 'src/app/shared/app-utility/app-utility.service';
...
@Component({
  selector: 'zabbix-storage-details',
  templateUrl: './zabbix-storage-details.component.html',
  styleUrls: ['./zabbix-storage-details.component.scss'],
  providers: [ZabbixStorageDetailsService]
})
export class ZabbixStorageDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private detailService: ZabbixStorageDetailsService,
  ...
  async getDeviceDetails() {
    ...
    this.detailService.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.getCredentials();
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.STORAGE_DEVICES, configured: res.monitoring.configured, isCluster: res.is_cluster, hasPureOs: res.is_purity, redfish: res.redfish }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
