...
import { GET_PRIVATE_CLOUD_FAST, POLL_PRIVATE_CLOUD_UPDATE, PRIVATE_CLOUD_BY_ID, PRIVATE_CLOUD_CONTAINERS_PODS, PRIVATE_CLOUD_CUSTOM_DEVICES, PRIVATE_CLOUD_ESXI_WIDGET_DATA, PRIVATE_CLOUD_WIDGET_DATA, VM_LIST_BY_PLATFORM } from 'src/app/shared/api-endpoint.const';
import { PlatFormMapping } from 'src/app/shared/app-utility/app-utility.service';
import { UserInfoService } from 'src/app/shared/user-info.service';
import { UsageData, UsageDataValueUnitType } from '../entities/usage-data.type';
...
export class SummaryService {
  ...
  getCloudDetails(pcId: string, platformType: string): Observable<PrivateCloudType> {
    const cloud = this.sharedSvc.getCloudNameForEndpoint(platformType);
    if (cloud === '') {
      return this.http.get<PrivateCloudType>(PRIVATE_CLOUD_BY_ID(pcId));
    } else {
      return this.http.get<PrivateCloudType>(`customer/managed/${cloud}/accounts/${pcId}/`);
    }
  }
  ...
  getUsageData(pcId: string, cloudName: string): Observable<UsageData> {
    if (cloudName === '') {
      return this.http.get<UsageData>(PRIVATE_CLOUD_WIDGET_DATA(pcId));
    } else {
      return this.http.get<UsageData>(`customer/managed/${cloudName}/accounts/${pcId}/widget_data/`);
    }
  }

  getVcenterDataStoreDetails(pcId: string): Observable<VCenterDataStore[]> {
    return this.http.get<VCenterDataStore[]>(`/customer/managed/vcenter/accounts/${pcId}/datastore_list/`);
  }
  ...
  getNutanixDataStoreDetails(pcId: string): Observable<NutanixDataStore[]> {
    return this.http.get<NutanixDataStore[]>(`/customer/managed/vcenter/accounts/${pcId}/datastore_list/`);
  }

  getEsxiUsageData(pcId: string, serverId: string): Observable<Map<string, EsxiHypervisorUsageData>> {
    return this.http.get<EsxiHypervisorUsageData>(PRIVATE_CLOUD_ESXI_WIDGET_DATA(pcId, serverId))
      .pipe(
        map((res: any) => {
          return new Map<string, EsxiHypervisorUsageData>().set(serverId, res);
        }),
        catchError((error: HttpErrorResponse) => {
          return of(new Map<string, EsxiHypervisorUsageData>().set(serverId, null));
        })
      );
  }
  ...
  pollForUsageDataUpdate(pcId: string, cloudName: string): Observable<TaskStatus> {
...
export interface EsxiHypervisorUsageData {
