...
import { ZabbixSwitchDetailsService } from './zabbix-switch-details.service';
import { CredentialMap } from 'src/app/app-constants';

@Component({
  selector: 'zabbix-switch-details',
  templateUrl: './zabbix-switch-details.component.html',
  styleUrls: ['./zabbix-switch-details.component.scss'],
  providers: [ZabbixSwitchDetailsService]
})
export class ZabbixSwitchDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private detailService: ZabbixSwitchDetailsService,
  ...
  async getDeviceDetails() {
    ...
    this.detailService.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.getCredentials();
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.SWITCHES, configured: res.monitoring.configured, redfish: res.redfish }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
