...
import { VmsZabbixMonitoringGraphItems } from '../vms-zabbix-monitoring.type';
import { VmsMonitoringGraphsViewdata, ZabbixVmsGraphCrudService } from './zabbix-vms-graph-crud.service';

@Component({
  selector: 'zabbix-vms-graph-crud',
  templateUrl: './zabbix-vms-graph-crud.component.html',
  styleUrls: ['./zabbix-vms-graph-crud.component.scss'],
  providers: [ZabbixVmsGraphCrudService]
})
export class ZabbixVmsGraphCrudComponent implements OnInit {
  private ngUnsubscribe = new Subject();
  deviceType: DeviceMapping;
  deviceId: string;
  viewData: VmsMonitoringGraphsViewdata[] = [];
  filteredViewData: VmsMonitoringGraphsViewdata[] = [];
  fieldsToFilterOn: string[] = ['name', 'graphType'];
  currentCriteria: SearchCriteria;

  selectedGraph: VmsMonitoringGraphsViewdata;
  graphItems: VmsZabbixMonitoringGraphItems[] = [];
  nonFieldErr: string = '';
  action: 'Add' | 'Edit';
  ...
  constructor(private graphService: ZabbixVmsGraphCrudService,
  ...
  ngOnInit() {
    setTimeout(() => {
      this.deviceType = (<DeviceTabData>this.storage.getByKey('device', StorageType.SESSIONSTORAGE)).deviceType;
  ...
  getGraphList() {
    this.graphService.getGraphList(this.deviceId, this.deviceType).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  createTicket(graph: VmsMonitoringGraphsViewdata) {
    this.ticketService.createTicket({
      subject: DATABASE_GRAPH_TICKET_SUBJECT(graph.name),
      metadata: DATABASE_GRAPH_TICKET_METADATA(this.deviceType, graph.name)
    }, this.deviceType);
  }

  getGraphItems() {
    this.graphService.getGraphItems(this.deviceId, this.deviceType).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.graphItems = res;
    }, (err: HttpErrorResponse) => {
    });
  }

  buildAddEditForm(graph?: VmsMonitoringGraphsViewdata) {
  ...
  confirmCreate() {
    if (this.graphForm.invalid) {
      ...
      if (this.selectedGraph) {
        this.graphService.updateGraph(this.deviceId, this.deviceType, this.selectedGraph.graphid, this.graphForm.getRawValue()).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
          this.graphModelRef.hide();
          this.spinner.stop('main');
        ...
        this.graphService.createGraph(this.deviceId, this.deviceType, this.graphForm.getRawValue()).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
          this.graphModelRef.hide();
          this.spinner.stop('main');
  ...
  editGraph(view: VmsMonitoringGraphsViewdata) {
    if (!view.canEdit) {
      return;
    }
    this.action = 'Edit';
  ...
  deleteGraph(view: VmsMonitoringGraphsViewdata) {
    if (!view.canDelete) {
      return;
    }
    this.selectedGraph = view;
  ...
  confirmDelete() {
    ...
    this.spinner.start('main');
    this.graphService.deleteGraph(this.deviceId, this.deviceType, this.selectedGraph.graphid).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.spinner.stop('main');
