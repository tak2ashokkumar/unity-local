...
import { DateRange, VmsGraphRange, VmsMonitoringGraphViewdata, ZabbixVmsGraphsService } from './zabbix-vms-graphs.service';

@Component({
  selector: 'zabbix-vms-graphs',
  templateUrl: './zabbix-vms-graphs.component.html',
  styleUrls: ['./zabbix-vms-graphs.component.scss'],
  providers: [ZabbixVmsGraphsService]
})
export class ZabbixVmsGraphsComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  deviceType: DeviceMapping;
  filterForm: FormGroup;
  formErrors: any;
  validationMessages: any;
  viewData: VmsMonitoringGraphViewdata[] = [];

  deviceid: string;
  graphList: VmsZabbixMonitoringGraph[] = [];
  graphRange = VmsGraphRange;
  graphView: VmsMonitoringGraphViewdata;
  now: any;
  dateRange: DateRange;
  ...
  constructor(private graphService: ZabbixVmsGraphsService,
  ...
  buildForm() {
    ...
    this.filterForm.get('period').valueChanges.pipe(takeUntil(this.ngUnsubscribe)).subscribe((val: VmsGraphRange) => {
  ...
  getGraph() {
    from(this.viewData).pipe(tap(e => setTimeout(() => { this.spinner.start(e.graphid) }, 0)),
      mergeMap(e => this.graphService.getGraph(this.deviceid, this.deviceType, e.graphid, this.filterForm.getRawValue())
