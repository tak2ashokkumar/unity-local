...
import { UnityAzureTopologyNode, UnityAzureTopologyLink, AzureTopologyType } from 'src/app/shared/SharedEntityTypes/azure.type';
...
@Injectable()
export class UnityAzureTopologyViewService {
  ...
  getDeviceNetwork(accountId: string): Observable<AzureTopologyType> {
    // return this.http.get<AzureTopologyType>(GET_DUMMY_JSON());
    return this.http.get<AzureTopologyType>(`/customer/managed/azure/accounts/${accountId}/topology/`);
  }

  getNodesandEdges(data: AzureTopologyType): { nodes: UnityAzureTopologyNode[], links: UnityAzureTopologyLink[] } {
    let nodes = data.nodes.filter((value, index, self) =>
      index === self.findIndex((t) => (t.uuid === value.uuid))
    )
    nodes.map(n => {
      n.displayType = n.resource_type;
      n.redirectLink = this.dataSvc.getAzureNodeRedirectLink(n);
      if (n.icon_path && n.icon_path != '') {
        n.icon = `${environment.assetsUrl}external-brand/azure/Icons/${n.icon_path}.svg`;
      }
    });
  ...
  getNode(n: UnityAzureTopologyNode): Node {
    let node: Node = { id: n.uuid, label: this.dataSvc.getAzureNodelabel(n) };
    node.shape = "image";
    node.imagePadding = 1;
    node.borderWidth = 1;
    node.size = 20;
  ...
  getNodes(nds: UnityAzureTopologyNode[]): Node[] {
    let nodes: Node[] = [];
    nds.map(n => {
  ...
  getEdge(ln: UnityAzureTopologyLink): Edge {
  ...
  getEdges(lnks: UnityAzureTopologyLink[]): Edge[] {
    let edges: Edge[] = [];
    lnks.map(ln => {
  ...
  convertToViewData(data: AzureTopologyType): UnityAzureTopologyViewData {
    let a = new UnityAzureTopologyViewData();
...
export class UnityAzureTopologyViewData {
  constructor() { }
  nodes: UnityAzureTopologyNode[] = [];
  edges: UnityAzureTopologyLink[] = [];
  nodeDataset: DataSet<Node> = null;
  edgeDataset: DataSet<Edge> = null;
  data: Data = null;
}

export interface UnityNetworkTopologyNodeViewData extends UnityAzureTopologyNode {
  faIcon?: string;
}
