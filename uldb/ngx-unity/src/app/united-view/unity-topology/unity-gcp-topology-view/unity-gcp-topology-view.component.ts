...
import { UnityGCPTopologyViewData, UnityGcpTopologyViewService } from './unity-gcp-topology-view.service';
...
import { GCPAccountResource } from 'src/app/shared/SharedEntityTypes/gcp.type';

@Component({
  selector: 'unity-gcp-topology-view',
  templateUrl: './unity-gcp-topology-view.component.html',
  styleUrls: ['./unity-gcp-topology-view.component.scss'],
  providers: [UnityGcpTopologyViewService, UnityTopologyoptionsService]
})
export class UnityGcpTopologyViewComponent implements OnInit {
  @Input() viewType: UnityTopologyViewType;
  private ngUnsubscribe = new Subject();

  networkViewData: UnityGCPTopologyViewData = new UnityGCPTopologyViewData();
  networkTopologyData: UnityGCPTopologyViewData;
  ...
  nodeDetailsRef: any;
  hoveredNode: GCPAccountResource;
  showNodeInfo: boolean = false;
  selectedNode: GCPAccountResource;
  selectedNodes: GCPAccountResource[] = [];
  selectedActiveNodes: GCPAccountResource[] = [];
  ...
  constructor(private svc: UnityGcpTopologyViewService,
  ...
  ngOnInit(): void {
    ...
    this.nodeDetailsRef = document.getElementById('gcp-node-details-wrapper');
  ...
  getDeviceNetwork() {
    this.svc.getDeviceNetwork(this.viewType.nodeId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.destroyNetwork();
      let networkData: UnityGCPTopologyViewData = this.svc.convertToViewData(res);
