import { Component, ElementRef, HostListener, Input, OnChanges, OnDestroy, OnInit, Renderer2, SimpleChanges, ViewChild, ViewEncapsulation } from '@angular/core';
...
import { UnityOciTopologyViewData, UnityOciTopologyViewService } from './unity-oci-topology-view.service';
import { OciAccountResource } from 'src/app/shared/SharedEntityTypes/oci.type';

@Component({
  selector: 'unity-oci-topology-view',
  templateUrl: './unity-oci-topology-view.component.html',
  styleUrls: ['./unity-oci-topology-view.component.scss'],
  providers: [UnityOciTopologyViewService, UnityTopologyoptionsService],
})
export class UnityOciTopologyViewComponent implements OnInit, OnDestroy, OnChanges {

  @Input() viewType: UnityTopologyViewType;
  private ngUnsubscribe = new Subject();

  networkViewData: UnityOciTopologyViewData = new UnityOciTopologyViewData();
  networkTopologyData: UnityOciTopologyViewData;
  ...
  nodeDetailsRef: any;
  hoveredNode: OciAccountResource;
  showNodeInfo: boolean = false;
  selectedNode: OciAccountResource;
  selectedNodes: OciAccountResource[] = [];
  selectedActiveNodes: OciAccountResource[] = [];
  ...
  constructor(
    private svc: UnityOciTopologyViewService,
  ...
  ngOnInit(): void {
    ...
    this.nodeDetailsRef = document.getElementById('oci-node-details-wrapper');
  ...
  getDeviceNetwork() {
    this.svc.getDeviceNetwork(this.viewType.nodeId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.destroyNetwork();
      let networkData: UnityOciTopologyViewData = this.svc.convertToViewData(res);
