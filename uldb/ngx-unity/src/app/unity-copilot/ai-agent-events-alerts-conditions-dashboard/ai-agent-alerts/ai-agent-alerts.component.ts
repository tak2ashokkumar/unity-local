...
@Component({
  ...
  styleUrls: ['./ai-agent-alerts.component.scss'],
  providers: [AiAgentAlertsService]
})
export class AiAgentAlertsComponent implements OnInit, OnDestroy {
  ...
  constructor(private alertSvc: AiAgentAlertsService,
  ...
  getAlerts() {
    ...
    this.alertSvc.getAlerts(this.currentCriteria, this.filterForm.getRawValue(), this.aiAgentConfig).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  resolveAlert(view: AiAgentAlertsViewdata) {
  ...
  disable(view: AiAgentAlertsViewdata) {
  ...
  resolveSuppressed(view: AiAgentAlertsViewdata) {
