...
export class AiAgentAlertsService {
  ...
  getAlertsSummary(aiAgentConfig: AiAgentConfigType) {
  ...
  getAlerts(criteria: SearchCriteria, filterData: any, aiAgentConfig: AiAgentConfigType) {
    ...
    return this.http.get<PaginatedResult<AiAgentAlerts>>(GET_AIOPS_ALERTS(), { params: params });
  }

  convertToViewdata(alerts: AiAgentAlerts[]) {
    let viewdata: AiAgentAlertsViewdata[] = [];
    alerts.forEach(al => {
      let view = new AiAgentAlertsViewdata();
      view.id = al.id;
      view.uuid = al.uuid;
  ...
  getSuppressedEvents(criteria: SearchCriteria, filterData: any) {
    ...
    return this.http.get<PaginatedResult<AiAgentSuppressedEvents>>(GET_AIOPS_SUPPRESSED_ALERTS(), { params: params });
  }
  ...
  onAcknowledge(alertId: string, formData: any, isSuppressed: boolean) {
    ...
    return this.http.post<AiAgentAlerts | AiAgentSuppressedEvents>(url, formData)
      .pipe(map(res => {
  ...
  disable(eventId: string): Observable<AiAgentSuppressedDisableTriggerType> {
    return this.http.post<AiAgentSuppressedDisableTriggerType>(`/customer/aiops/events/${eventId}/disable_trigger/`, {});
  }
