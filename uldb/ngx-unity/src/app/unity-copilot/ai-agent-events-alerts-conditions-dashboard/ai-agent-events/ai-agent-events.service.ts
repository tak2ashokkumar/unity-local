...
import { AiAgentEventResolveType, AiAgentEvents } from './ai-agent-events.type';
import { GET_AIOPS_EVENTS } from 'src/app/shared/api-endpoint.const';
...
export class AiAgentEventsService {
  ...
  getEvents(criteria: SearchCriteria, filterData: any, AiAgentConfig: AiAgentConfigType) {
    ...
    if (!filterData.device_type || filterData.device_type.length === 0) {
      AiAgentConfig.deviceTypesForApi.forEach(type => {
        params = params.append('device_type', type);
      });
    }
    return this.http.get<PaginatedResult<AiAgentEvents>>(GET_AIOPS_EVENTS(), { params: params });
  }

  getDropdownData(aiAgentType: string) {
    return of(AIOPS_DEVICE_TYPES.filter(d => d.aiAgentType == aiAgentType));
  }

  convertDetailsToViewdata(events: AiAgentEvents[]) {
    let viewdata: AiAgentEventsViewData[] = [];
    events.forEach(event => {
      let view = new AiAgentEventsViewData();
      view.id = event.id;
      ...
      view.acknowledgedTooltipMsg = `Ack by: ${view.acknowledgedBy}<br>` + `Ack Msg: ${insertLineBreaks(view.acknowledgedComment, 10, 35)
        }<br>` + `Ack At: ${view.acknowledgedTime} `;
  ...
  onAcknowledge(eventId: string, formData: any): Observable<AiAgentEvents> {
    return this.http.post<AiAgentEvents>(`/customer/aiops/events/${eventId}/acknowledge/`, formData)
      .pipe(map(res => {
  ...
  resolve(eventId: string): Observable<AiAgentEventResolveType> {
    return this.http.post<AiAgentEventResolveType>(`/customer/aiops/events/${eventId}/resolve/`, {});
  }
}
...
export class AiAgentEventsViewData {
  constructor() { }
  id: number;
  uuid: string;
  deviceName: string;
