...
import { aimlEventsColumnMapping, AIMLEventsViewData, EventsFilterFormData, NetworkAgentEventsService } from './network-agent-events.service';
...
import { AIMLEventsSummary } from './network-agent-events.type';
...
@Component({
  selector: 'network-agent-events',
  templateUrl: './network-agent-events.component.html',
  styleUrls: ['./network-agent-events.component.scss'],
  providers: [NetworkAgentEventsService]
})
export class NetworkAgentEventsComponent implements OnInit, OnDestroy {
  ...
  constructor(private eventSvc: NetworkAgentEventsService,
