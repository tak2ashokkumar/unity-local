...
import { AiObservabilityGpuServiceDetailsService } from './ai-observability-gpu-service-details.service';
...
@Component({
  selector: 'ai-observability-gpu-service-details',
  templateUrl: './ai-observability-gpu-service-details.component.html',
  styleUrls: ['./ai-observability-gpu-service-details.component.scss'],
  providers: [AiObservabilityGpuServiceDetailsService]
})
export class AiObservabilityGpuServiceDetailsComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  constructor(private svc: AiObservabilityGpuServiceDetailsService,
  ...
  getDeviceDetails() {
    this.svc.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.GPU_SERVICE, configured: true }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
