...
import { AiObservabilityLlmDetailsService } from './ai-observability-llm-details.service';

@Component({
  selector: 'ai-observability-llm-details',
  templateUrl: './ai-observability-llm-details.component.html',
  styleUrls: ['./ai-observability-llm-details.component.scss'],
  providers: [AiObservabilityLlmDetailsService]
})
export class AiObservabilityLlmDetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private svc: AiObservabilityLlmDetailsService,
  ...
  getDeviceDetails() {
    this.svc.getDeviceDetails(this.deviceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.view = res;
      this.storageService.put('device', { name: res.name, deviceType: DeviceMapping.LLM_SERVICE, configured: true }, StorageType.SESSIONSTORAGE);
      this.buildForm();
    }, err => {
      this.spinner.stop('main');
