...
import { AIMLAlerts, AIMLAlertsSummary, AIMLSuppressedDisableTriggerType, AIMLSuppressedEvents, AIMLSuppressedResolveType } from './aiml-alerts.type';
...
export class AimlAlertsService {
  ...
  convertToSuppressedEventsViewdata(alerts: AIMLSuppressedEvents[]) {
    let viewdata: AIMLAlertsViewdata[] = [];
    alerts.map(al => {
      let view = new AIMLAlertsViewdata();
      view.id = al.id;
      ...
      view.alertTime = al.event_datetime ? this.utilSvc.toUnityOneDateFormat(al.event_datetime) : 'N/A';
      // view.source = al.source;
