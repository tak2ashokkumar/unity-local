...
import { AimlConditionDetailsService } from '../aiml-condition-details/aiml-condition-details.service';
...
@Component({
  selector: 'aiml-conditions',
  templateUrl: './aiml-conditions.component.html',
  styleUrls: ['./aiml-conditions.component.scss'],
  providers: [AimlConditionsService]
})
export class AimlConditionsComponent implements OnInit, OnDestroy {
  ...
  summaryViewData: AIMLConditionsSummary;
  ...
  selectedCondition: AIMLConditionDetails;
  ...
  isBlinking = false;
  blinkInterval: any;
  matchedAlert: AIMLConditionAlertsViewData;
  ...
  constructor(
    ...
    //     if (event.url.split('/').pop() == 'conditions') {
  ...
  // viewEventDetails(alert: AIMLHostBasedAlertsData) {
  //   this.conditionSvc.getAlertDetails(alert.alertId).pipe(takeUntil(this.ngUnsubscribe)).subscribe((res) => {
  //     // this.selectedEvent = this.conditionSvc.convertToEventDetailsViewdata(res);
  //     // this.handleTimelineWidth();
  //   }, (err) => {
  ...
  isActive(tab: TabData) {
    // if (this.router.url.match('/conditions/' + tab.url)) {
  ...
  goTo(tab: TabData) {
    ...
    // else if (this.tabdetails == 'AI Analysis') {
    //   if (!this.analysisViewData) {
    //     setTimeout(() => {
    //       this.spinner.start('AnalysisData');
    //     }, 100);
    //   }
