...
import { ADDING_EVENT_TYPE, DELETE_EVENT_TYPE, GET_EVENT_TYPE, UPDATE_EVENT_TYPE } from 'src/app/shared/api-endpoint.const';
...
@Injectable()
export class SourceEventTypesService {
  ...
  getEventTypes(criteria: SearchCriteria): Observable<PaginatedResult<EventType>> {
    return this.tableService.getData<PaginatedResult<EventType>>(GET_EVENT_TYPE(), criteria);
  }
  ...
  keywordUpdate(arr: string[], uuid: string) {
    return this.http.patch(UPDATE_EVENT_TYPE(uuid), { 'keywords': arr });
  }

  eventAdd(formData: any) {
    return this.http.post<EventType>(ADDING_EVENT_TYPE(), formData);
  }

  eventUpdate(obj: any, uuid: string) {
    return this.http.patch<EventType>(UPDATE_EVENT_TYPE(uuid), obj);
  }

  eventDelete(uuid: string) {
    return this.http.delete(DELETE_EVENT_TYPE(uuid));
  }
}
