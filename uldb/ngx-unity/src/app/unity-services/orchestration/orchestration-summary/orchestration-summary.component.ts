...
export class OrchestrationSummaryComponent implements OnInit, OnDestroy {
  ...
  constructor(private svc: OrchestrationSummaryService,
    ...
    private aiSvc: UnityChatbotService,
    private userInfoSvc: UserInfoService,
    private appSvc: AppMainService,
  ...
  getTaskWidgetData(): void {
    this.spinnerSvc.start(this.taskWidget.loader);
    this.taskWidget.byStatusChartData = null;
    this.taskWidget.byTargetChartData = null;
    this.taskWidget.byScriptTypeChartData = null;
    this.svc.getTaskWidgetData()
      .pipe(takeUntil(this.ngUnsubscribe), finalize(() => this.stopLoader(this.taskWidget.loader)))
      .subscribe(res => {
        ...
        this.notificationSvc.error(new Notification('Failed to get Task Details'));
      });
  }

  getWorkflowWidgetData(): void {
    this.spinnerSvc.start(this.workflowWidget.loader);
    this.workflowWidget.byStatusChartData = null;
    this.workflowWidget.byCategoryChartData = null;
    this.svc.getWorkflowWidgetData()
      .pipe(takeUntil(this.ngUnsubscribe), finalize(() => this.stopLoader(this.workflowWidget.loader)))
      .subscribe(res => {
        ...
        this.notificationSvc.error(new Notification('Failed to get Workflow Details'));
      });
  }

  onExecutionSummaryWidgetFormChanged(event: any): void {
    this.executionSummaryWidget.formData = event;
    this.getExecutionSummaryWidgetData();
    this.userInfoSvc.isInsightsEnabled && this.getInsight();
  }

  getExecutionSummaryWidgetData(): void {
    this.spinnerSvc.start(this.executionSummaryWidget.loader);
    ...
    this.svc.getExecutionSummaryWidgetData(this.executionSummaryWidget?.formData?.from, this.executionSummaryWidget?.formData?.to)
      .pipe(takeUntil(this.ngUnsubscribe), finalize(() => this.stopLoader(this.executionSummaryWidget.loader)))
      .subscribe(res => {
        ...
        this.notificationSvc.error(new Notification('Failed to get Execution Summary Details'));
      });
  }

  onExecutionsOverviewWidgetFormChanged(event: any): void {
  ...
  getExecutionsOverviewData(): void {
    this.spinnerSvc.start(this.executionsOverviewViewData.loader);
    this.executionsOverviewViewData.successRateChartData = null;
    this.executionsOverviewViewData.efficiencyChartData = null;
    this.svc.getExecutionsOverviewData(this.executionsOverviewViewData?.formData?.from, this.executionsOverviewViewData?.formData?.to)
      .pipe(takeUntil(this.ngUnsubscribe), finalize(() => this.stopLoader(this.executionsOverviewViewData.loader)))
      .subscribe(res => {
        this.executionsOverviewViewData.successRateChartData = this.svc.convertToExecutionsBySuccessRateChartData(res.execution_success_rate);
        this.executionsOverviewViewData.efficiencyChartData = this.svc.convertToExecutionsByEfficiencyChartData(res.execution_efficiency);
      }, () => {
        this.notificationSvc.error(new Notification('Failed to get Executions Details'));
      });
  }

  getRecentFailureData(): void {
    this.svc.getRecentFailureData()
      ...
      .subscribe(res => {
        ...
        this.notificationSvc.error(new Notification('Failed to get Recent Failures'));
      });
  }

  getUpcomingExecutionData(): void {
    this.svc.getUpcomingExecutionData()
      .pipe(takeUntil(this.ngUnsubscribe), finalize(() => this.cdr.markForCheck()))
      .subscribe(res => {
        this.upcomingExecutionViewData = this.svc.convertToUpcomingExecutionViewData(res);
      }, () => {
        this.notificationSvc.error(new Notification('Failed to get Upcoming Executions'));
      });
  }

  onChartClick(chartInstance: any): void {
    chartInstance.on('click', (params: any) => {
      this.goTo(chartInstance.getOption(), params)
    });
  }

  goTo(option: EChartsOption, params: any): void {
    switch (option.targetEntity) {
      case 'Task': this.goToTasks(option.chartName as string, params); break;
      case 'Workflow': this.goToWorkflows(option.chartName as string, params); break;
      case 'Execution': this.goToExecutions(option.chartName as string, params); break;
      default: return;
    }
  }

  goToTasks(chartName?: string, params?: any): void {
    switch (chartName) {
  ...
  goToWorkflows(chartName?: string, params?: any): void {
    switch (chartName) {
  ...
  goToExecutions(chartName?: string, _params?: any): void {
    switch (chartName) {
  ...
  execute(data: any): void {
    if (data.type === "Task") {
      ...
      this.aiSvc.onChatTrigger$.next(true);
      this.appSvc.$assistantData.next({ sourceName: 'workflow', apiUrl: `/rest/orchestration/agentic_workflow/${data.uuid}/chat/`, entityId: data.uuid, entity: data.name });
    } else {
  ...
  redirectToTaskOrWorkflowEdit(data: any): void {
    if (data.type === "Task") {
  ...
  redirectToExecutionPage(data: RecentFailureViewModel): void {
    if (data.type === "Task") {
  ...
  getInsight(): void {
    this.aiSvc.onDevopsFilterChange(this.executionSummaryWidget?.formData?.from, this.executionSummaryWidget?.formData?.to);
  }

  trackByFailure(_index: number, item: RecentFailureViewModel): string {
    return item.uuid;
  }

  trackByUpcoming(_index: number, item: UpccomingExecutionViewModel): string {
    return item.taskOrWorkflowId;
  }
