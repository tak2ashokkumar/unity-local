...
import { DEVICES_FAST_BY_DEVICE_TYPE, GET_ADVANCED_DISCOVERY_CREDENTIALS, ORCHESTRATION_INPUT_TEMPLATE } from 'src/app/shared/api-endpoint.const';
...
import { Account, Tag, WorkflowDetails, WorkflowMetaData, WorkflowTask } from '../orchestration-workflow-execution/orchestration-workflow-execution.type';
...
@Injectable()
export class OrchestrationWorkflowExecutionService {
  ...
  //   buildCloudForm(param: WorkflowDetails): FormGroup {
  ...
  //   buildHostForm(param: WorkflowDetails): FormGroup {
  //     let form = this.builder.group({
  //       'ip': ['', [ipListValidator()]],
  //       'host_type': [''],
  //       'host': [[]],
  ...
  //   buildLocalForm(param: WorkflowDetails): FormGroup {
  //     let form = this.builder.group({
  //       'templates': this.builder.array([])
  //     });
  //     if (param.templates) {
  ...
  //   resetLocalFormErrors() {
  //     return {
  //       'templates':{}
  //     };
  //   }

  //   localFormValidationMessages = {
  //     'templates' : {}
  //   }
  ...
  //           inputsArray.push(
  //             this.builder.group({
  //               template_name: new FormControl({ value: input.template_name, disabled: true }),
  //               attribute: new FormControl({ value: input.attribute, disabled: true }),
  ...
  //               param_name: new FormControl({ value: input.param_name, disabled: false })
  //             })
  //           );
  //         } else {
  //           inputsArray.push(
  ...
  //   executeTask(data: any, workflowId: string) {
  //     return this.http.post(`/orchestration/workflows/${workflowId}/execute/`, data);
  //   }
  // }
  ...
  getTemplateData(): Observable<InputTemplateModel> {
    return this.http.get<InputTemplateModel>(ORCHESTRATION_INPUT_TEMPLATE());
  }
  ...
  buildCloudForm(param: TaskParamsType, taskId: string) {
    let form = this.builder.group({
      'cloud_account': ['', [Validators.required]],
      'cred': [param.cred],
      // 'credentials': ['', [Validators.required]],
      'inputs': this.builder.array([]),
      ...
      JSON.stringify(param.inputs) !== '{}' ? (param.inputs as TerraFormParams[]).forEach(p => {
        if (p.param_type === 'Input Template') {
          const inputGroup = this.builder.group({
            ...
            attribute: new FormControl({ value: p.attribute, disabled: true }),
            param_type: new FormControl({ value: p.param_type, disabled: false }),
            default_value: new FormControl({ value: p.default_value, disabled: false }, [Validators.required]),
            template: new FormControl({ value: p.template, disabled: false }),
            param_name: new FormControl({ value: p.param_name, disabled: false }),
            label: new FormControl({ value: p.param_name }),
            filters: new FormControl({})
  ...
  getHost(search: string, tag?: string, deviceType?: string, dc?: string, subType?: string, publicCloud?: string, privateCloud?: string): Observable<any> {
    ...
    if (dc) {
      params = params.append('datacenter', dc);
    }
    if (subType) {
      params = params.append('sub_type', subType);
    }
  ...
  getCloudWithImg() {
    return this.http.get<any>(`orchestration/workflows/get_metadata/`);
  }

  buildLocalForm(param: TaskParamsType, taskId: string) {
    let form = this.builder.group({
      'inputs': this.builder.array([]),
      // 'templates': this.builder.array([])
    });
