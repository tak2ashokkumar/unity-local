...
import { Hosts, OrchestrationWorkflowScheduleService, deviceTypes } from './orchestration-workflow-schedule.service';
import { Observable, of, Subject } from 'rxjs';
import { Account, DropdownWithImage, Tag, WorkflowDetails, WorkflowTask, scheduleType } from './orchestration-workflow-schedule.type';
...
import { TemplateOption } from '../orchestration-workflow-execution/orchestration-workflow-execution.type';
import { TerraFormParams } from '../orchestration-workflow-execution/orchestration-workflow-execution.service';
import { environment } from 'src/environments/environment';
import { CloudNameType } from '../../orchestration-tasks/orchestration-tasks-schedule/orchestration-tasks-type';

@Component({
  selector: 'orchestration-workflow-schedule',
  templateUrl: './orchestration-workflow-schedule.component.html',
  styleUrls: ['./orchestration-workflow-schedule.component.scss'],
  providers: [OrchestrationWorkflowScheduleService]
})
export class OrchestrationWorkflowScheduleComponent implements OnInit, OnDestroy {
  ...
  // constructor(private workflowScheduleService: OrchestrationWorkflowScheduleService,
  ...
  //   this.workflow = null;
  //   this.workflowScheduleService.getWorkflowDetails(this.workflowId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  //     this.workflow = res;
  //     this.parametersList = res.tasks;
  //     this.workflowDetailsData = this.workflowScheduleService.converToViewData(res);
  //     if (this.workflow.target_type && this.workflow.target_type == 'Cloud') {
  //       this.getAccounts(this.workflow.cloud_type);
  //       this.buildCloudForm();
  //       this.hostOrCloud = 'Cloud';
  ...
  //       this.getTags();
  //       this.buildHostForm();
  //       this.hostOrCloud = 'Host';
  //       this.activeForm = 'Host';
  //       this.getScheduleData();
  //       this.spinner.stop('main');
  //     }
  ...
  //   this.accountList = [];
  //   this.workflowScheduleService.getCloudList(cloudType).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  // getCredentials() {
  //   this.credentialList = [];
  //   this.workflowScheduleService.getCredentials().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  // buildCloudForm() {
  //   this.cloudForm = this.workflowScheduleService.buildCloudForm(this.workflow);
  //   this.cloudFormErrors = this.workflowScheduleService.resetCloudFormErrors();
  //   this.cloudFormValidationMessages = this.workflowScheduleService.cloudFormValidationMessages;
  ...
  //         this.workflowScheduleService.getTemplateOption(template.uuid).pipe(takeUntil(this.ngUnsubscribe)).subscribe(options => {
  ...
  //             this.workflowScheduleService.getTemplateOptionWithDep(dependentTemplate.uuid, selectedValue, accountId.val).pipe(takeUntil(this.ngUnsubscribe)).subscribe((dependentOptions: TemplateOption[]) => {
  ...
  // getDatacenters() {
  //   this.datacenterList = [];
  //   this.workflowScheduleService.getDataCenters().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  // getClouds() {
  //   this.cloudList = [];
  //   this.workflowScheduleService.getClouds().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  ...
  // getTags() {
  //   this.tags = [];
  //   this.workflowScheduleService.getTags().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  //     this.tags = res;
  //   }, (err: HttpErrorResponse) => {
  ...
  // buildHostForm() {
  //   this.hostForm = this.workflowScheduleService.buildHostForm(this.workflow);
  //   this.hostFormErrors = this.workflowScheduleService.resetHostFormErrors();
  //   this.hostFormValidationMessages = this.workflowScheduleService.hostFormValidationMessages;
  //   this.manageHostForm();
  // }
  ...
  //   this.WorkflowName = this.workflow.workflow_name;
  //   if (this.workflow.templates && this.workflow.templates.length > 0) {
  //     this.workflow.templates.forEach(template => {
  ...
  //         });
  //       }
  //       const templateControl = this.hostForm.get('templates').get(template.label);
  //       templateControl.valueChanges.pipe(takeUntil(this.ngUnsubscribe)).subscribe((selectedValue: string) => {
  //         let row = { dep: template.dependency_name, val: selectedValue };
  //         this.tempArr.push(row);
  //         const dependentTemplate = this.templateList.find(t => t.dependency_name === template.name);
  //         const accountId = this.tempArr.find(val => val.dep == '');
  //         if (dependentTemplate) {
  //           this.workflowScheduleService.getTemplateOptionWithDep(dependentTemplate.uuid, selectedValue, accountId.val).pipe(takeUntil(this.ngUnsubscribe)).subscribe((dependentOptions: TemplateOption[]) => {
  //             this.templateOptions[dependentTemplate.label] = dependentOptions;
  //             this.hostForm.get('templates').get(dependentTemplate.label).enable();
  //           });
  //         }
  //         // }
  //       });
  ...
  //       this.workflowScheduleService.getIpAddress(ip).subscribe(response => {
  ...
  //   this.hosts = [];
  //   this.filteredHosts = [];
  //   this.workflowScheduleService.getHost(tag, deviceType, dc, subType, publicCloud, privateCloud).pipe(takeUntil(this.ngUnsubscribe)).subscribe(response => {
  //     this.hosts = response;
  ...
  //     this.workflowDetailsData.forEach(data => {
  //       data.taskFormErrors = this.workflowScheduleService.resetTaskFormErrors();
  //       if (data.type == 'Ansible Playbook' || data.type == 'Terraform Script') {
  ...
  //         }
  //       } else {
  //         if (data.taskForm && Object.keys(data.taskForm).length) {
  //           Object.keys(data.taskForm).forEach(key => {
  //             data.taskFormErrors.inputs[key] = '';
  //             data.taskValidationMessages.inputs[key] = {};
  //             data.taskValidationMessages.inputs[key]['required'] = `${key} is required`;
  //           });
  //         }
  //       }
  //     });
  ...
  // getScheduleData() {
  //   this.spinner.start('main');
  //   this.workflowScheduleService.getScheduleData(this.workflowId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  //     this.scheduleData = data;
  //     this.buildScheduleForm();
  //     if (this.scheduleData?.schedule_meta) {
  //       if (this.targetType == 'cloud' || this.targetType == 'local') {
  //         if (data.schedule_meta.templates) {
  //           Object.entries(data.schedule_meta.templates).forEach(([key, value]) => {
  //             const control = (this.cloudForm.get('templates') as FormGroup).get(key);
  //             if (control) {
  ...
  //         this.workflowDetailsData.forEach(workflowDetail => {
  //           const taskForm = workflowDetail.taskForm;
  //           this.scheduleData.tasks.forEach(task => {
  //             const correspondingTask = this.scheduleData.tasks.find(task => task.uuid === workflowDetail.taskForm.get('uuid').value);

  //             taskForm.removeControl('inputs');
  //             const inputsArray = (taskForm.get('inputs') as FormArray) || new FormArray([]);

  //             correspondingTask.inputs.forEach(input => {
  ...
  //                 });
  //               }
  //               inputsArray.push(inputGroup);
  //             });
  //             taskForm.setControl('inputs', inputsArray);
  //           });
  ...
  //         if (data.schedule_meta.cred == 'local') {
  ...
  //         }
  //         if (data.schedule_meta.cloud_cred == 'local') {
  //           this.cloudForm.get('cloud_cred').setValue('local');
  //           this.cloudForm.get('cloud_credentials').setValue(data.schedule_meta.cloud_credentials);
  //         } else {
  //           this.cloudForm.get('cloud_cred').setValue('my_cred');
  //           this.cloudForm.get('cloud_username').setValue(data.schedule_meta.cloud_username);
  //           this.cloudForm.get('cloud_password').setValue(data.schedule_meta.cloud_password);
  //         }
  //         this.spinner.stop('main');
  //       } else {
  ...
  //         if (data.schedule_meta.templates) {
  //           Object.entries(data.schedule_meta.templates).forEach(([key, value]) => {
  //             const control = (this.hostForm.get('templates') as FormGroup).get(key);
  //             if (control) {
  //               control.setValue(value);
  //             }
  //           });
  ...
  //           this.scheduleData.tasks.forEach(task => {
  //             const correspondingTask = this.scheduleData.tasks.find(task => task.uuid === workflowDetail.taskForm.get('uuid').value);
  //             task.inputs.forEach(taskInput => {
  //               taskForm.removeControl('inputs');
  //               taskForm.addControl('inputs', new FormGroup({}));

  //               correspondingTask.inputs.forEach(input => {
  //                 if (input.param_type === 'Input Template') {
  //                   (taskForm.get('inputs') as FormGroup).addControl(input.param_name, new FormGroup({
  //                     template_name: new FormControl({ value: input.template_name, disabled: false }),
  //                     attribute: new FormControl({ value: input.attribute, disabled: false }),
  //                     param_type: new FormControl({ value: input.param_type, disabled: false }),
  //                     default_value: new FormControl({ value: input.default_value, disabled: false }),
  ...
  //                     default_value: new FormControl(input.default_value, [Validators.required])
  //                   });
  //                   (taskForm.get('inputs') as FormGroup).addControl(input.param_name, defaultValueGroup);
  //                 }
  //               });
  //             });
  //           });
  ...
  //           this.hostForm.get('username').setValue(data.schedule_meta.username);
  //           this.hostForm.get('password').setValue(data.schedule_meta.password);
  //         }
  ...
  // onSubmitCloudForm() {
  //   if ((this.workflow.target_type && this.workflow.target_type == 'Cloud') || (this.workflow.target_type && this.workflow.target_type == 'Local')) {
  ...
  //   if ((this.workflow.target_type && this.workflow.target_type == 'Cloud') || (this.workflow.target_type && this.workflow.target_type == 'Local')) {
  //     obj = Object.assign({}, this.cloudForm.getRawValue(), this.scheduleSvc.getFormValue());
  //   } else {
  //     const { cred, credentials, username, password, parameters } = this.hostForm.getRawValue();
  //     obj = Object.assign({}, this.hostForm.getRawValue(), this.scheduleSvc.getFormValue());
  //   }
  //   obj['tasks'] = [];
  //   if (this.workflowDetailsData.length) {
  //     this.workflowDetailsData.forEach(data => {
  ...
  //   }
  //   this.spinner.start('main');
  //   this.workflowScheduleService.scheduleWorkflow(obj, this.workflowId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
  //     this.spinner.stop('main');
  //     this.notification.success(new Notification('Workflow Schedule details submitted succesfuly.'));
  //     this.goBack();
  //   }, (err: HttpErrorResponse) => {
  //     this.spinner.stop('main');
  ...
  constructor(private svc: OrchestrationWorkflowScheduleService,
    private router: Router,
    private route: ActivatedRoute,
    private spinner: AppSpinnerService,
    private notification: AppNotificationService,
    ...
    this.route.paramMap.subscribe((params: ParamMap) => {
      this.taskId = params.get('workflowId');
      this.targetType = params.get('targetType')?.toLowerCase();
      this.categoryId = params.get('categoryId')
      console.log(this.taskId, "Task Id", this.categoryId, "category Id")
    });
  }
  ...
  buildCloudForm() {
    ...
    this.svc.getTaskParamsById(this.taskId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(param => {
      ...
      this.requiredCredentials = param.required_credentials;
      ...
      this.notification.error(new Notification('Error while fetching Workflow parameters. Please try again!!'));
      this.spinner.stop('main');
    });
  }
  ...
  onSubmitHostForm() {
    console.log(this.hostForm, "host form")
    if (this.hostForm.invalid) {
      this.hostFormErrors = this.utilService.validateForm(this.hostForm, this.hostFormValidationMessages, this.hostFormErrors);
