...
import { CategoriesData, CategoriesViewData, SourceTaskViewData, TaskViewData, WorkflowCategory, WorkflowGroup } from './wf-dynamic-left.type';
import { environment } from 'src/environments/environment';

@Injectable({
  providedIn: 'root'
})
export class WfDynamicLeftService {

  private workflowGroupsCache: WorkflowGroup[] | null = null;
  ...
  */
  ...
  getWorkflowGroups(): Observable<{ groups: WorkflowGroup[] }> {
    return this.http.get<{ groups: WorkflowGroup[] }>('/api/orchestration/v1/workflows/');
  }

  getDynamicGroupItems(endpoint: string): Observable<WorkflowCategory[]> {
    return this.http.get<WorkflowCategory[]>(endpoint);
  }

  setWorkflowGroupsCache(data: WorkflowGroup[]) {
    this.workflowGroupsCache = data;
  }

  getWorkflowGroupsCache(): WorkflowGroup[] | null {
    return this.workflowGroupsCache;
  }

  clearWorkflowGroupsCache() {
    this.workflowGroupsCache = null;
  }
}
