...
import { OpenstackVmBackupService, OpenstackBackupViewData } from './openstack-vm-backup.service';
...
@Component({
  selector: 'openstack-vm-backup',
  templateUrl: './openstack-vm-backup.component.html',
  styleUrls: ['./openstack-vm-backup.component.scss'],
  providers: [OpenstackVmBackupService]
})
export class OpenstackVmBackupComponent implements OnInit, OnDestroy {
  ...
  constructor(private backupService: VmBackupService,
    private osService: OpenstackVmBackupService,
  ...
  getVMsFromDB() {
    ...
    this.backupService.getVMMigrationsFromDB<OpenStackMigrationType>(this.currentCriteria, this.cloud.platform_type)
  ...
  backupOpenstack(data: OpenstackBackupViewData, backup_type: string) {
  ...
  awsBackupVM() {
    this.backupAWSFormErrors = this.osService.resetAWSBackupFormErrors();
    this.backupAWSValidationMessages = this.osService.validationMessagesAWSBackup;
    this.backupAWSForm = this.osService.createAWSBackupForm();
  ...
  confirmAWSBackup() {
    if (this.backupAWSForm.invalid) {
      ...
      this.osService.confirmVmwareBackup(Object.assign({
  ...
  azureBackupVM() {
    this.backupAzureFormErrors = this.osService.resetAzureBackupFormErrors();
    this.backupAzureValidationMessages = this.osService.validationMessagesAzureBackup;
    this.backupAzureForm = this.osService.createAzureBackupForm();
  ...
  backUpHistory(view: OpenstackBackupViewData) {
    this.osService.osBackupHistory(view.id).subscribe(status => {
      this.backuphistory = status.openstack_backup_list;
