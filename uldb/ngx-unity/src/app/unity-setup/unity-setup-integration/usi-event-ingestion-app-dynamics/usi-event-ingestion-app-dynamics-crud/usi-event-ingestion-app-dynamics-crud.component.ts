...
import { UsiEventIngestionAppDynamicsCrudService } from './usi-event-ingestion-app-dynamics-crud.service';
import { AppDynamicsAccountsViewData } from '../usi-event-ingestion-app-dynamics.service';

@Component({
  selector: 'usi-event-ingestion-app-dynamics-crud',
  templateUrl: './usi-event-ingestion-app-dynamics-crud.component.html',
  styleUrls: ['./usi-event-ingestion-app-dynamics-crud.component.scss']
})
export class UsiEventIngestionAppDynamicsCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: AppDynamicsAccountsViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionAppDynamicsCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('App Dynamics Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('App Dynamics Instance added successfully'));
  ...
  confirmDelete() {
    ...
    this.crudService.deleteInstance(this.instanceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notification.success(new Notification('App Dynamics Instance deleted successfully.'));
      this.onCrud.emit(CRUDActionTypes.DELETE)
    }, err => {
      this.notification.error(new Notification('Failed to delete Oci Instance!! Please try again.'));
    });
  }
