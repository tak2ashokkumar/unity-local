...
import { UsiEventIngestionAwsCrudService } from './usi-event-ingestion-aws-crud.service';
import { Notification } from 'src/app/shared/app-notification/notification.type';
import { HttpErrorResponse } from '@angular/common/http';
import { AwsInstanceViewData } from '../usi-event-ingestion-aws.service';

@Component({
  selector: 'usi-event-ingestion-aws-crud',
...
export class UsiEventIngestionAwsCrudComponent implements OnInit {
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Aws Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Aws Instance added successfully'));
  ...
  confirmDelete() {
    ...
    this.crudService.deleteInstance(this.instanceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notification.success(new Notification('Aws Instance deleted successfully.'));
      this.onCrud.emit(CRUDActionTypes.DELETE)
    }, err => {
      this.notification.error(new Notification('Failed to delete Aws Instance!! Please try again.'));
    });
  }
