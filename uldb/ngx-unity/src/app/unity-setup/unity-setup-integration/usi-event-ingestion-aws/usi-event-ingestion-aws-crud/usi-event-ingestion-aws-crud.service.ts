...
import { AwsInstanceType } from './usi-event-ingestion-aws.type';

@Injectable()
export class UsiEventIngestionAwsCrudService {
  ...
  constructor(private builder: FormBuilder,
    ...
    buildForm(uuid: string): Observable<FormGroup> {
      if (uuid) {
        return this.http.get<AwsInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
          map(data => {
    ...
    addInstance(obj: AwsInstanceType) {
      return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
    }
  
    updateInstance(obj: AwsInstanceType, uuid: string) {
