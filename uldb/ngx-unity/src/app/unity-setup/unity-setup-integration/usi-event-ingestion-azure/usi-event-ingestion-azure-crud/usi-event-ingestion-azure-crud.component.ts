...
import { UsiEventIngestionAzureCrudService } from './usi-event-ingestion-azure-crud.service';
import { AzureInstanceViewData } from '../usi-event-ingestion-azure.service';

@Component({
  selector: 'usi-event-ingestion-azure-crud',
  templateUrl: './usi-event-ingestion-azure-crud.component.html',
  styleUrls: ['./usi-event-ingestion-azure-crud.component.scss']
})
export class UsiEventIngestionAzureCrudComponent implements OnInit,OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: AzureInstanceViewData;
  instanceId: string;
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Azure Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Azure Instance added successfully'));
