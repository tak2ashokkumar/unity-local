...
import { AzureInstanceType } from './usi-event-ingestion-azure-crud.type';
import { ADD_NAGIOS_INSTANCE, DELETE_NAGIOS_INSTANCE, EDIT_NAGIOS_INSTANCE } from 'src/app/shared/api-endpoint.const';

@Injectable()
export class UsiEventIngestionAzureCrudService {
  ...
  buildForm(uuid: string): Observable<FormGroup> {
    if (uuid) {
      return this.http.get<AzureInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
        map(data => {
  ...
  addInstance(obj: AzureInstanceType) {
    return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
  }

  updateInstance(obj: AzureInstanceType, uuid: string) {
