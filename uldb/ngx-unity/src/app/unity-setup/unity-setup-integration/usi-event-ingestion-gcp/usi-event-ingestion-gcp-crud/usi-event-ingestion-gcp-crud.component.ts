...
import { GcpInstanceViewData } from '../usi-event-ingestion-gcp.service';
import { UsiEventIngestionGcpCrudService } from './usi-event-ingestion-gcp-crud.service';

@Component({
  selector: 'usi-event-ingestion-gcp-crud',
...
export class UsiEventIngestionGcpCrudComponent implements OnInit {
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Gcp Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Gcp Instance added successfully'));
  ...
  confirmDelete() {
    ...
    this.crudService.deleteInstance(this.instanceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notification.success(new Notification('Gcp Instance deleted successfully.'));
      this.onCrud.emit(CRUDActionTypes.DELETE)
    }, err => {
      this.notification.error(new Notification('Failed to delete Gcp Instance!! Please try again.'));
    });
  }
