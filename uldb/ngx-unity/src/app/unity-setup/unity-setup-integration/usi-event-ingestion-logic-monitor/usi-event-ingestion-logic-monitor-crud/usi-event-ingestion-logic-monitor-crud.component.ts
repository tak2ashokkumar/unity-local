...
import { LogicMonitorAccountViewData } from '../usi-event-ingestion-logic-monitor.service';
import { UsiEventIngestionLogicMonitorCrudService } from './usi-event-ingestion-logic-monitor-crud.service';

@Component({
  selector: 'usi-event-ingestion-logic-monitor-crud',
...
export class UsiEventIngestionLogicMonitorCrudComponent implements OnInit {
  ...
  nonFieldErr: string = '';
  instance: LogicMonitorAccountViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionLogicMonitorCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Logic Monitoring Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Logic Monitoring Instance added successfully'));
  ...
  confirmDelete() {
    ...
    this.crudService.deleteInstance(this.instanceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notification.success(new Notification('Logic Monitoring Instance deleted successfully.'));
