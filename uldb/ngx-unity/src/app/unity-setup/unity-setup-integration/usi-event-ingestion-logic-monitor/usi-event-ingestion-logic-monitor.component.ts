...
import { LogicMonitorAccountViewData, UsiEventIngestionLogicMonitorService } from './usi-event-ingestion-logic-monitor.service';
...
@Component({
  selector: 'usi-event-ingestion-logic-monitor',
  templateUrl: './usi-event-ingestion-logic-monitor.component.html',
  styleUrls: ['./usi-event-ingestion-logic-monitor.component.scss'],
  providers: [UsiEventIngestionLogicMonitorService]
})
export class UsiEventIngestionLogicMonitorComponent implements OnInit, OnDestroy {

  public tabItems: TabData[] = [{
    name: 'Logic Monitor Event Ingestion',
    url: '/setup/integration/logicmonitor'
  }];

  private ngUnsubscribe = new Subject();
  viewData: LogicMonitorAccountViewData[] = [];
  selectedView: LogicMonitorAccountViewData;
  ...
  constructor(private crudService: UsiEventIngestionCommonService,
    ...
    private appDynamicsService: UsiEventIngestionLogicMonitorService) { }
  ...
  getAccountsData() {
    this.appDynamicsService.getAccounts().pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  ...
  testPayloadModal(view: LogicMonitorAccountViewData) {
    this.isSubmit = true;
  ...
  goToSettings(source?: string) {
    const queryParams = { source: '1300' };
    this.router.navigate(['services/aiml/rules/logicmonitor'], { queryParams });
  }
