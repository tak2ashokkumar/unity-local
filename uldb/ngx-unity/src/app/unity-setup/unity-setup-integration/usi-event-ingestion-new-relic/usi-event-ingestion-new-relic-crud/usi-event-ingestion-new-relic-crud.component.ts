...
import { NewRelicAccountViewData } from '../usi-event-ingestion-new-relic.service';
import { UsiEventIngestionNewRelicCrudService } from './usi-event-ingestion-new-relic-crud.service';
...
@Component({
  selector: 'usi-event-ingestion-new-relic-crud',
  templateUrl: './usi-event-ingestion-new-relic-crud.component.html',
  styleUrls: ['./usi-event-ingestion-new-relic-crud.component.scss']
})
export class UsiEventIngestionNewRelicCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: NewRelicAccountViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionNewRelicCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('New Relic Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('New Relic Instance added successfully'));
