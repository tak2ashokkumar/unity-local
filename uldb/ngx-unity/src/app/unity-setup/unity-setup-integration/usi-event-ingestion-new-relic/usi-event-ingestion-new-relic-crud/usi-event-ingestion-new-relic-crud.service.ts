...
import { NewRelicInstanceType } from './usi-event-ingestion-new-relic-crud.type';
...
@Injectable()
export class UsiEventIngestionNewRelicCrudService {
  ...
  buildForm(uuid: string): Observable<FormGroup> {
    if (uuid) {
      return this.http.get<NewRelicInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
        map(data => {
  ...
  addInstance(obj: NewRelicInstanceType) {
    return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
  }

  updateInstance(obj: NewRelicInstanceType, uuid: string) {
