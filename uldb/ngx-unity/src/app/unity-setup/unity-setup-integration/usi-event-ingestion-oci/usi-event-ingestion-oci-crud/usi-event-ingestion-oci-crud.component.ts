...
import { UsiEventIngestionOciCrudService } from './usi-event-ingestion-oci-crud.service';
import { AppSpinnerService } from 'src/app/shared/app-spinner/app-spinner.service';
import { AppNotificationService } from 'src/app/shared/app-notification/app-notification.service';
import { takeUntil } from 'rxjs/operators';
import { Notification } from 'src/app/shared/app-notification/notification.type';
import { HttpErrorResponse } from '@angular/common/http';
import { OciInstanceViewData } from '../usi-event-ingestion-oci.service';

@Component({
  selector: 'usi-event-ingestion-oci-crud',
  templateUrl: './usi-event-ingestion-oci-crud.component.html',
  styleUrls: ['./usi-event-ingestion-oci-crud.component.scss']
})
export class UsiEventIngestionOciCrudComponent implements OnInit, OnDestroy {
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Oci Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Oci Instance added successfully'));
  ...
  confirmDelete() {
    ...
    this.crudService.deleteInstance(this.instanceId).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notification.success(new Notification('Oci Instance deleted successfully.'));
