...
import { UsiEventIngestionOpsrampCrudService } from './usi-event-ingestion-opsramp-crud.service';
import { AppSpinnerService } from 'src/app/shared/app-spinner/app-spinner.service';
import { AppNotificationService } from 'src/app/shared/app-notification/app-notification.service';
import { takeUntil } from 'rxjs/operators';
import { Notification } from 'src/app/shared/app-notification/notification.type';
import { HttpErrorResponse } from '@angular/common/http';
import { OpsrampAccountViewData } from '../usi-event-ingestion-opsramp.service';

@Component({
  selector: 'usi-event-ingestion-opsramp-crud',
  templateUrl: './usi-event-ingestion-opsramp-crud.component.html',
  styleUrls: ['./usi-event-ingestion-opsramp-crud.component.scss']
})
export class UsiEventIngestionOpsrampCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: OpsrampAccountViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionOpsrampCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Opsramp Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Opsramp Instance added successfully'));
