...
import { OpsrampInstanceType } from './usi-event-ingestion-opsramp-crud.type';

@Injectable()
export class UsiEventIngestionOpsrampCrudService {
  ...
  buildForm(uuid: string): Observable<FormGroup> {
    if (uuid) {
      return this.http.get<OpsrampInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
        map(data => {
  ...
  addInstance(obj: OpsrampInstanceType) {
    return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
  }

  updateInstance(obj: OpsrampInstanceType, uuid: string) {
