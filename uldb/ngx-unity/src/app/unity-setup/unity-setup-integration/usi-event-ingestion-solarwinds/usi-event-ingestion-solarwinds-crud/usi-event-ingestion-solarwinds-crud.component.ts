...
import { UsiEventIngestionSolarwindsCrudService } from './usi-event-ingestion-solarwinds-crud.service';
import { AppSpinnerService } from 'src/app/shared/app-spinner/app-spinner.service';
import { AppNotificationService } from 'src/app/shared/app-notification/app-notification.service';
import { takeUntil } from 'rxjs/operators';
import { Notification } from 'src/app/shared/app-notification/notification.type';
import { HttpErrorResponse } from '@angular/common/http';
import { SolarwindsInstanceViewData } from '../usi-event-ingestion-solarwinds.service';

@Component({
  selector: 'usi-event-ingestion-solarwinds-crud',
  templateUrl: './usi-event-ingestion-solarwinds-crud.component.html',
  styleUrls: ['./usi-event-ingestion-solarwinds-crud.component.scss']
})
export class UsiEventIngestionSolarwindsCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: SolarwindsInstanceViewData;
  instanceId: string;
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('SolarWinds Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('SolarWinds Instance added successfully'));
