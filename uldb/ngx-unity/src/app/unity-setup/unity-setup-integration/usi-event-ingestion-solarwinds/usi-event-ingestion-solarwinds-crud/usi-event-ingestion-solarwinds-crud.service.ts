...
import { SolarwindsInstanceType } from './usi-event-ingestion-solarwinds-crud.type';
...
export class UsiEventIngestionSolarwindsCrudService {
  ...
  buildForm(uuid: string): Observable<FormGroup> {
    if (uuid) {
      return this.http.get<SolarwindsInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
        map(data => {
  ...
  addInstance(obj: SolarwindsInstanceType) {
    return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
  }

  updateInstance(obj: SolarwindsInstanceType, uuid: string) {
