...
import { SolarwindsInstanceType } from './usi-event-ingestion-solarwinds-crud/usi-event-ingestion-solarwinds-crud.type';

@Injectable()
export class UsiEventIngestionSolarwindsService {
  ...
  getInstances() {
    ...
    params = params.append('source', 'SolarWinds');
    return this.http.get<SolarwindsInstanceType[]>(GET_NAGIOS_INSTANCE(), { params: params });
  }

  convertToViewData(data: SolarwindsInstanceType[]): SolarwindsInstanceViewData[] {
    let viewData: SolarwindsInstanceViewData[] = [];
    data.forEach(a => {
      let d: SolarwindsInstanceViewData = new SolarwindsInstanceViewData();
      d.id = a.id;
      d.uuid = a.uuid;
      d.name = a.name;
...
export class SolarwindsInstanceViewData {
  constructor() { }
  uuid: string;
  id: string;
  name: string;
