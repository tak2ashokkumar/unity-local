...
import { UsiEventIngestionZabbixCrudService } from './usi-event-ingestion-zabbix-crud.service';
...
import { ZabbixAccountViewData } from '../usi-event-ingestion-zabbix.service';
import { Notification } from 'src/app/shared/app-notification/notification.type';
import { HttpErrorResponse } from '@angular/common/http';

@Component({
  selector: 'usi-event-ingestion-zabbix-crud',
  templateUrl: './usi-event-ingestion-zabbix-crud.component.html',
  styleUrls: ['./usi-event-ingestion-zabbix-crud.component.scss']
})
export class UsiEventIngestionZabbixCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: ZabbixAccountViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionZabbixCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Zabbix Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Zabbix Instance added successfully'));
