...
import { ZabbixInstanceType } from './zabbix-instance-type';
import { map } from 'rxjs/operators';
import { NoWhitespaceValidator } from 'src/app/shared/app-utility/app-utility.service';
import { ADD_NAGIOS_INSTANCE, DELETE_NAGIOS_INSTANCE, DELETE_ZABBIX_INSTANCE, EDIT_NAGIOS_INSTANCE, EDIT_ZABBIX_INSTANCE } from 'src/app/shared/api-endpoint.const';
import { RxwebValidators } from '@rxweb/reactive-form-validators';

@Injectable()
export class UsiEventIngestionZabbixCrudService {
  ...
  buildForm(uuid: string): Observable<FormGroup> {
    if (uuid) {
      return this.http.get<ZabbixInstanceType>(`customer/aiops/event-ingestion-instance/${uuid}/`).pipe(
        map(data => {
  ...
  addInstance(obj: ZabbixInstanceType) {
    return this.http.post(ADD_NAGIOS_INSTANCE(), obj);
  }

  updateInstance(obj: ZabbixInstanceType, uuid: string) {
