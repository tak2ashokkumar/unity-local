...
@Component({
  selector: 'usi-event-ingestion-zabbix',
  templateUrl: './usi-event-ingestion-zabbix.component.html',
...
export class UsiEventIngestionZabbixComponent implements OnInit {
  ...
  public tabItems: TabData[] = [{
    ...
    url: '/setup/integration/zabbix'
  }];

  private ngUnsubscribe = new Subject();
  viewData: ZabbixAccountViewData[] = [];
  ...
  constructor(private crudService: UsiEventIngestionCommonService,
    ...
    private zabbixService: UsiEventIngestionZabbixService,
  ...
  getAccountsData() {
    this.zabbixService.getAccounts().pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      ...
      this.spinner.stop('main');
      this.notification.error(new Notification('Zabbix to get Accounts'));
    });
  }
