...
import { DynatraceAccountViewData } from '../usi-event-ingestion-dynatrace.service';
import { UsiEventIngestionDynatraceCrudService } from './usi-event-ingestion-dynatrace-crud.service';
...
@Component({
  selector: 'usi-event-ingestion-dynatrace-crud',
  templateUrl: './usi-event-ingestion-dynatrace-crud.component.html',
  styleUrls: ['./usi-event-ingestion-dynatrace-crud.component.scss']
})
export class UsiEventIngestionDynatraceCrudComponent implements OnInit, OnDestroy {
  ...
  nonFieldErr: string = '';
  instance: DynatraceAccountViewData;
  instanceId: string;

  constructor(private crudService: UsiEventIngestionDynatraceCrudService,
    ...
    if (this.form.invalid) {
      ...
      if (this.action == 'Edit') {
        this.crudService.updateInstance(this.form.getRawValue(), this.instanceId).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Dynatrace Instance updated successfully'));
        ...
        this.crudService.addInstance(this.form.getRawValue()).pipe(takeUntil(this.ngUnsubscribe))
          .subscribe(res => {
            this.modalRef.hide();
            this.notification.success(new Notification('Dynatrace Instance added successfully'));
