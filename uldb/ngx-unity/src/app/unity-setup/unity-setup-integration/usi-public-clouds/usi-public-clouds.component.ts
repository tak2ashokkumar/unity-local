...
import { PublicCloudAccountViewData, PublicCloudScheduleHistoryViewData, UsiPublicCloudsService } from './usi-public-clouds.service';
import { AppUtilityService } from 'src/app/shared/app-utility/app-utility.service';
import { SharedCreateTicketService } from 'src/app/shared/shared-create-ticket/shared-create-ticket.service';
import { GCP_ACCOUNT_TICKET_METADATA, OCI_ACCOUNT_TICKET_METADATA, TICKET_SUBJECT } from 'src/app/shared/create-ticket.const';

@Component({
  selector: 'usi-public-clouds',
...
export class UsiPublicCloudsComponent implements OnInit {
  ...
  count: number;
  viewData: PublicCloudAccountViewData[] = [];
  selectedView: PublicCloudAccountViewData;
  isSubmit: boolean = true;
  scheduleHistoryData: PublicCloudScheduleHistoryViewData[] = [];
  ...
  scheduleHistoryView: PublicCloudAccountViewData;

  @ViewChild('accessKey') accessKey: ElementRef;
  accessKeyModalRef: BsModalRef;

  constructor(private service: UsiPublicCloudsService,
  ...
  getCloudName() {
    ...
    if (currentUrl.includes('azure')) {
      this.cloudNameForEndpoint = 'azure';
      this.cloudNameForDisplay = 'Azure';
    } else if (currentUrl.includes('gcp')) {
      this.cloudNameForEndpoint = 'gcp';
      this.cloudNameForDisplay = 'GCP';
    } else if (currentUrl.includes('oracle')) {
      this.cloudNameForEndpoint = 'oci';
      this.cloudNameForDisplay = 'Oracle';
    }else if (currentUrl.includes('aws')) {
      this.cloudNameForEndpoint = 'aws';
      this.cloudNameForDisplay = 'AWS';
    }
    this.getAccounts();
  }
  ...
  getAccounts() {
    ...
    this.service.getAccounts(this.currentCriteria, this.cloudNameForEndpoint).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      ...
      this.viewData = this.service.convertToViewData(res.results, this.cloudNameForEndpoint);
      this.spinner.stop('main');
  ...
  editAccount(view: PublicCloudAccountViewData) {
    this.router.navigate([view.uuid, 'edit'], { relativeTo: this.route });
  }

  deleteAccount(view: PublicCloudAccountViewData) {
  ...
  createTicket(view: PublicCloudAccountViewData) {
  ...
  createTicketOracle(view: PublicCloudAccountViewData) {
    this.ticketService.createTicket({
      subject: TICKET_SUBJECT(view.name, view.region), metadata: OCI_ACCOUNT_TICKET_METADATA(view.name)
    });
  }

  copyKey(value: string, view: PublicCloudAccountViewData) {
    if (!view.ingestEvent) {
      return;
    }
    try {
  ...
  testPayload(view: PublicCloudAccountViewData) {
    if (!view.ingestEvent) {
      return;
    }
    this.selectedView = view;
  ...
  syncNow(view: PublicCloudAccountViewData) {
  ...
  getScheduleHistory(view?: PublicCloudAccountViewData) {
    this.spinner.start('main');
    if (view) {
  ...
  loadResources(view: PublicCloudAccountViewData) {
    if (view?.discoverResources) {
