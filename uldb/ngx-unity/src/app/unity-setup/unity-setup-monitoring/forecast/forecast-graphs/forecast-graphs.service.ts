...
import { DeviceZabbixMonitoringGraph, ZabbixMonitoringGraph } from 'src/app/united-cloud/shared/zabbix-graphs/zabbix-graphs.type';
import moment from 'moment';

@Injectable()
export class ForecastGraphsService {
  constructor(private http: HttpClient,
    private buiilder: FormBuilder,
    private tableService: TableApiServiceService,
    private util: AppUtilityService) { }

  getGraphList(deviceType: DeviceMapping, deviceId: string, itemId: string): Observable<ZabbixMonitoringGraph[]> {
    return this.http.get<ZabbixMonitoringGraph[]>(`customer/alert_prediction/devices/${deviceId}/items/${itemId}/graphs/`);
  ...
  getGraph(deviceId: string, itemId: string, graphId: string, formData: any): Observable<{ [key: string]: string }> {
    const format = new DateRange().format;
    const params = new HttpParams().set('from', moment(formData['from']).format(format)).set('to', moment(formData['to']).format(format));
    return this.http.get<{ [key: string]: string }>(`/customer/alert_prediction/devices/${deviceId}/graph_image/${graphId}/`, { params: params })
  }
}
