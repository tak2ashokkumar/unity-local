...
import { HypervisorCRUDManufacturer, HypervisorCRUDOperatingSystem, HypervisorCRUDPrivateCloudFast } from 'src/app/united-cloud/shared/entities/hypervisor-crud.type';
import { AdvancedDiscoveryHypervisorsService, DevDisHypervisorViewdata, DeviceDiscoveryHypervisorFormData } from './advanced-discovery-hypervisors.service';

@Component({
  selector: 'advanced-discovery-hypervisors',
  templateUrl: './advanced-discovery-hypervisors.component.html',
  styleUrls: ['./advanced-discovery-hypervisors.component.scss'],
  providers: [AdvancedDiscoveryHypervisorsService]
})
export class AdvancedDiscoveryHypervisorsComponent implements OnInit {
  @ViewChildren('fold') folds: QueryList<ElementRef>;
  private ngUnsubscribe = new Subject();
  viewData: DevDisHypervisorViewdata[] = [];
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private deviceDiscoveryService: AdvancedDiscoveryHypervisorsService) { }
  ...
  getHypervisor() {
    this.deviceDiscoveryService.getHypervisor().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.viewData = this.deviceDiscoveryService.convertToViewData(res.results);
      this.getDropDownData();
    }, err => {
      this.spinner.stop('main');
      this.notification.error(new Notification('Failed to load Hypervisor. Tryagain later.'));
    });
  }
  ...
  getDropDownData() {
    this.deviceDiscoveryService.getDropdownData().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      ...
      this.spinner.stop('main');
      this.notification.error(new Notification('Failed to load Hypervisor Data. Tryagain later.'))
    })
  }

  getModels(data: DevDisHypervisorViewdata, manufacturer: string) {
  ...
  formSubscriptions() {
    this.viewData.forEach(data => {
      ...
      data.form.get('cabinet.id').valueChanges.pipe(takeUntil(this.ngUnsubscribe)).subscribe((val: string) => {
        data.cabinet = val ? data.cabinets.find(m => m.id == Number(val)).name : '';
        data.form.get('position').setValue('');
        if (val) {
          data.form.get('position').enable();
        } else {
  ...
  getCabinets(data: DevDisHypervisorViewdata) {
  ...
  getPrivateClouds(data: DevDisHypervisorViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
  ...
  openRow(obj: DevDisHypervisorViewdata) {
  ...
  open(obj: DevDisHypervisorViewdata) {
    if (!obj.openEnabled) {
      return;
    }
    if (obj.models) {
      ...
      let m: HypervisorCRUDManufacturer = null;
        ...
        for (let i = 0; i < this.viewData.length; i++) {
          if (this.viewData[i].uniqueId == key) {
            if (errorRes[key]['saved']) {
              ...
              this.viewData[i].formErrors = this.deviceDiscoveryService.resetHypervisorsFormErrors();
  ...
  submit() {
    ...
    if (count == changedForms.length) {
      let arr = [];
      changedForms.filter(view => view.openEnabled).forEach(view => arr.push(<DeviceDiscoveryHypervisorFormData>view.form.getRawValue()));
      this.spinner.start('main');
      this.deviceDiscoveryService.saveAll(arr).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
        ...
        this.notification.success(new Notification('Hypervisor onboarded successfully.'));
