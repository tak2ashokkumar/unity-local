...
import { DeviceDiscoverySwitchFormData } from '../../unity-setup-device-discovery/device-discovery-switches/device-discovery-switches.service';
import { AdvancedDiscoverySwitchesService, DevDisSwitchViewdata } from './advanced-discovery-switches.service';

@Component({
  selector: 'advanced-discovery-switches',
  templateUrl: './advanced-discovery-switches.component.html',
  styleUrls: ['./advanced-discovery-switches.component.scss'],
  providers: [AdvancedDiscoverySwitchesService]
})
export class AdvancedDiscoverySwitchesComponent implements OnInit {
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private deviceDiscoveryService: AdvancedDiscoverySwitchesService) { }
  ...
  getSwitches() {
    this.deviceDiscoveryService.getSwitches().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      ...
      this.notification.error(new Notification('Failed to load Switches. Try again later.'));
    });
  }
  ...
  getCabinets(data: DevDisSwitchViewdata) {
  ...
  getPrivateClouds(data: DevDisSwitchViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
