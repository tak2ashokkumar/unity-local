...
import { ExcelOnBoardingFirewallsService, ExcelOnBoardingFirewallViewdata } from './excel-on-boarding-firewalls.service';
import { DeviceDiscoveryAgentConfigurationType } from '../../advanced-discovery-connectivity/agent-config.type';

@Component({
  selector: 'excel-on-boarding-firewalls',
  templateUrl: './excel-on-boarding-firewalls.component.html',
  styleUrls: ['./excel-on-boarding-firewalls.component.scss'],
  providers: [ExcelOnBoardingFirewallsService]
})
export class ExcelOnBoardingFirewallsComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  viewData: ExcelOnBoardingFirewallViewdata[] = [];
  manufacturers: Array<FirewallCRUDManufacturer> = [];
  collectors: DeviceDiscoveryAgentConfigurationType[] = [];

  models: { [key: string]: FirewallCRUDModel[] } = {};
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private xlSvc: ExcelOnBoardingFirewallsService) {
  ...
  getFirewalls() {
    this.xlSvc.getFirewalls(this.fileIds).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  ...
  getModels(data: ExcelOnBoardingFirewallViewdata) {
    data.models = [];
  ...
  getCabinets(data: ExcelOnBoardingFirewallViewdata) {
  ...
  getPrivateClouds(data: ExcelOnBoardingFirewallViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
