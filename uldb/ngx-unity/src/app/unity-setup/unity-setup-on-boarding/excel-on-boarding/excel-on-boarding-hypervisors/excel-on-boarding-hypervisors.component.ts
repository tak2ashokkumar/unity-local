...
import { ExcelOnBoardingHypervisorsService, ExcelOnBoardingHypervisorViewdata } from './excel-on-boarding-hypervisors.service';
import { DeviceDiscoveryAgentConfigurationType } from '../../advanced-discovery-connectivity/agent-config.type';

@Component({
  selector: 'excel-on-boarding-hypervisors',
  templateUrl: './excel-on-boarding-hypervisors.component.html',
  styleUrls: ['./excel-on-boarding-hypervisors.component.scss'],
  providers: [ExcelOnBoardingHypervisorsService]
})
export class ExcelOnBoardingHypervisorsComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  viewData: ExcelOnBoardingHypervisorViewdata[] = [];
  manufacturers: Array<HypervisorCRUDManufacturer> = [];
  operatingSystems: Array<HypervisorCRUDOperatingSystem> = [];
  models: { [key: string]: HypervisorCRUDModel[] } = {};
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private xlSvc: ExcelOnBoardingHypervisorsService) {
  ...
  getHypervisors() {
    this.xlSvc.getHypervisors(this.fileIds).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  ...
  getModels(data: ExcelOnBoardingHypervisorViewdata) {
    data.models = [];
  ...
  getCabinets(data: ExcelOnBoardingHypervisorViewdata) {
  ...
  getPrivateClouds(data: ExcelOnBoardingHypervisorViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
  ...
  submit() {
    ...
    if (count == changedForms.length) {
      ...
      this.xlSvc.saveAll(arr).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
        this.spinner.stop('main');
        this.getHypervisors();
        this.notification.success(new Notification('Hypervisors onboarded successfully.'));
        // this.nxtPrvSvc.continueNextPrev();
      }, (err: HttpErrorResponse) => {
        this.handleError(err.error);
      });
