...
import { ExcelOnBoardingLoadbalancersService, ExcelOnBoardingLoadbalancerViewdata } from './excel-on-boarding-loadbalancers.service';
import { DeviceDiscoveryAgentConfigurationType } from '../../advanced-discovery-connectivity/agent-config.type';

@Component({
  selector: 'excel-on-boarding-loadbalancers',
  templateUrl: './excel-on-boarding-loadbalancers.component.html',
  styleUrls: ['./excel-on-boarding-loadbalancers.component.scss'],
  providers: [ExcelOnBoardingLoadbalancersService]
})
export class ExcelOnBoardingLoadbalancersComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  viewData: ExcelOnBoardingLoadbalancerViewdata[] = [];
  ...
  models: { [key: string]: LoadBalancerCRUDModel[] } = {};
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private xlSvc: ExcelOnBoardingLoadbalancersService) {
  ...
  getLoadbalancers() {
    this.xlSvc.getLoadbalancers(this.fileIds).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  ...
  getModels(data: ExcelOnBoardingLoadbalancerViewdata) {
    data.models = [];
  ...
  getCabinets(data: ExcelOnBoardingLoadbalancerViewdata) {
  ...
  getPrivateClouds(data: ExcelOnBoardingLoadbalancerViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
