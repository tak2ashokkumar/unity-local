...
import { ExcelOnBoardingMacService, ExcelOnBoardingMacViewdata } from './excel-on-boarding-mac.service';
import { DeviceDiscoveryAgentConfigurationType } from '../../advanced-discovery-connectivity/agent-config.type';

@Component({
  selector: 'excel-on-boarding-mac',
  templateUrl: './excel-on-boarding-mac.component.html',
  styleUrls: ['./excel-on-boarding-mac.component.scss'],
  providers: [ExcelOnBoardingMacService]
})
export class ExcelOnBoardingMacComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  viewData: ExcelOnBoardingMacViewdata[] = [];
  manufacturers: Array<MacMiniCRUDManufacturer> = [];
  operatingSystems: Array<MacMiniCRUDOperatingSystem> = [];
  models: { [key: string]: MacMiniCRUDModel[] } = {};
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private xlSvc: ExcelOnBoardingMacService) {
  ...
  getMacs() {
    this.xlSvc.getMacs(this.fileIds).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
      this.viewData = this.xlSvc.converToViewdata(data);
      this.getDropDownData();
      this.spinner.stop('main');
    }, err => {
      this.notification.error(new Notification('Error while fetching mac devices list'));
      this.spinner.stop('main');
    });
  }
  ...
  getModels(data: ExcelOnBoardingMacViewdata) {
    data.models = [];
  ...
  getCabinets(data: ExcelOnBoardingMacViewdata) {
  ...
  getPrivateClouds(data: ExcelOnBoardingMacViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
  ...
  submit() {
    ...
    if (count == changedForms.length) {
      ...
      this.xlSvc.saveAll(arr).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
        this.spinner.stop('main');
        this.getMacs();
        this.notification.success(new Notification('Mac devices onboarded successfully.'));
        // this.nxtPrvSvc.continueNextPrev();
      }, (err: HttpErrorResponse) => {
        this.handleError(err.error);
      });
