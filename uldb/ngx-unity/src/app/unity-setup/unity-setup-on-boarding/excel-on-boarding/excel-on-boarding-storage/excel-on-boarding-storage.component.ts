...
import { ExcelOnBoardingStorageService, ExcelOnBoardingStorageViewdata } from './excel-on-boarding-storage.service';
import { DeviceDiscoveryAgentConfigurationType } from '../../advanced-discovery-connectivity/agent-config.type';

@Component({
  selector: 'excel-on-boarding-storage',
  templateUrl: './excel-on-boarding-storage.component.html',
  styleUrls: ['./excel-on-boarding-storage.component.scss'],
  providers: [ExcelOnBoardingStorageService]
})
export class ExcelOnBoardingStorageComponent implements OnInit, OnDestroy {
  private ngUnsubscribe = new Subject();
  viewData: ExcelOnBoardingStorageViewdata[] = [];
  ...
  models: { [key: string]: StorageCRUDModel[] } = {};
  fileIds: string[] = [];
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private xlSvc: ExcelOnBoardingStorageService) {
  ...
  getStorages() {
    this.xlSvc.getStorages(this.fileIds).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
  ...
  getModels(data: ExcelOnBoardingStorageViewdata) {
    data.models = [];
  ...
  getCabinets(data: ExcelOnBoardingStorageViewdata) {
  ...
  getPrivateClouds(data: ExcelOnBoardingStorageViewdata) {
    let selectedDcId = data.form.get('datacenter.uuid').value;
    data.clouds = [];
  ...
  submit() {
    ...
    if (count == changedForms.length) {
      ...
      this.xlSvc.saveAll(arr).pipe(takeUntil(this.ngUnsubscribe)).subscribe(data => {
        this.spinner.stop('main');
        this.getStorages();
        this.notification.success(new Notification('Storage devices onboarded successfully.'));
      }, (err: HttpErrorResponse) => {
        this.handleError(err.error);
      });
    }
