...
import { AgentConfigurationService, AgentConfigurationViewData } from './agent-configuration.service';
...
export class AgentConfigurationComponent implements OnInit, OnDestroy {
  ...
  constructor(private agentService: AgentConfigurationService,
  ...
  confirmDelete() {
    ...
    this.agentService.deleteConfig(this.selectedAgent).pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.notificationService.success(new Notification('Removed configuration sucessfully'));
      this.getConfigurations();
    }, err => {
