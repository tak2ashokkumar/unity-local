...
import { DevDisFireWallViewdata, DeviceDiscoveryFirewallFormData, DeviceDiscoveryFirewallsService } from './device-discovery-firewalls.service';

@Component({
  selector: 'device-discovery-firewalls',
  templateUrl: './device-discovery-firewalls.component.html',
  styleUrls: ['./device-discovery-firewalls.component.scss'],
  providers: [DeviceDiscoveryFirewallsService]
})
export class DeviceDiscoveryFirewallsComponent implements OnInit, OnDestroy {
  ...
  privateclouds: Array<FirewallCRUDPrivateCloudFast> = [];
  ...
  constructor(private utilService: AppUtilityService,
    ...
    private fwSvc: DeviceDiscoveryFirewallsService) { }
  ...
  getFirewalls() {
    this.fwSvc.getFirewalls().pipe(takeUntil(this.ngUnsubscribe)).subscribe(res => {
      this.viewData = this.fwSvc.convertToViewData(res);
