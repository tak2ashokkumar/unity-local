...
import { ApprovalWorkflows } from '../../unity-setup-policy.type';

@Injectable()
export class UnitySetupPolicyPoliciesCrudService {
  ...
  buildForm(data: any): FormGroup {
    if (data) {
      ...
      }, { validators: notifyUserOrGroupValidator() });
...
export function notifyUserOrGroupValidator(): ValidatorFn {
  return (group: AbstractControl): ValidationErrors | null => {
    const notificationEnabled = group.get('notification_enabled')?.value;
    const notifyUsers = group.get('notify_users')?.value;
    const notifyGroups = group.get('notify_groups')?.value;

    const usersValid = Array.isArray(notifyUsers) && notifyUsers.length > 0;
    const groupsValid = Array.isArray(notifyGroups) && notifyGroups.length > 0;

    if (notificationEnabled) {
      if (!usersValid && !groupsValid) {
        return { notifyUserOrGroupRequired: true };
      }
    }
